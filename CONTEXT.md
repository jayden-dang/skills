...
- **Home module** — the single module whose folders own all of a feature's owned paths; a
  feature whose owned paths span two modules (or none) has no home and is unassigned.
- **Shard** — the generated per-module file (`modules/<CODE>.md`) listing the features homed
  to that module; regenerated by `check-graph --harvest`, never hand-edited.
- **Index** — the top-level `GRAPH.md` in module-manifest mode, listing the modules and the
  cross-module shared surface. Not the Reverse index: the Index points at shards.

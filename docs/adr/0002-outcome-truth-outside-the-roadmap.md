...
block collided with its intent-only ownership split, reusing `record-verdict`
meant reopening a caller set deliberately closed to `land-branch` and
`cut-release`, and a conversational report let a one-time judgment die on compaction.

...
`finish-branch` already owns every branch crossing behind a verify/trace/acceptance
gate, a verbatim five-option menu, and a record-before-crossing rule, but it authors
no commit message and no PR body. **Decision:** `prepare-change` authors content and
mutates only local git, handing a file-based package to `finish-branch`, which alone
...
model-invoked skill from invoking user-invoked `/file-issues`.

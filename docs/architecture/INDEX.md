...
- **ARCH-4** Requirement IDs (`CODE-N.M`) and architecture IDs (`ARCH-N`) are immutable once defined: never renumber or reuse; retire only by strikethrough; every **docs-side** citation (`Satisfies:`, `_Requirements:`, `Respects:`, and bold definitions) MUST use the same greppable string as the definition. Application source and tests MUST NOT be required to embed these IDs.

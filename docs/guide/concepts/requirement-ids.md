...
It is not a heading in a document. It is a string that appears in a test tag, a commit trailer, an issue body, a design section, a task footer, and a changelog line — and the [audit-trace check](traceability.md) reports a failure when those uses disagree with its definition.
...
The pattern the [audit-trace check](../resources/scripts.md#the-trace-check) matches is `\b([A-Z][A-Z0-9]{1,11})-(\d+)\.(\d+)(?![.\d])`. Two details in there are deliberate. There is no trailing `\b`, because a markdown italics footer ends with `_` — a word character — which would silently drop the last ID on the line. And the negative lookahead `(?![.\d])` guards against matching a prefix of a longer number.
...
Tier-1 fixes that no feature owns may instead be defined in `docs/specs/fixes.md`, a shared home the audit-trace check reads the same way.
...
The per-layer test conventions are not hardcoded. [`configure-repo`](../skills/configure-repo.md) asks which convention each test layer in *this* repo uses and records the answers in `docs/agents/project.md`; [`test-first`](../skills/test-first.md) and the [audit-trace check](../resources/scripts.md#the-trace-check) both read from there. The only hard requirement is that the convention be **greppable**.
...
The audit-trace check treats a struck-through ID as **undefined**. That is the clever part: the moment you retire a requirement, every test and task still citing it surfaces immediately as an **E1** error ("cites unknown requirement"). Retirement cannot be done quietly.

New requirements take the next free number under their story. [`realign-spec`](../skills/realign-spec.md) enforces both rules as its Iron Rules, and treats the orphans that strikethrough surfaces as *decisions to put to the user*, not cleanup to perform silently.

## The checks that enforce it

The [audit-trace check](../resources/scripts.md#the-trace-check) reports three errors and two warnings. Any error is a failure the invoking skill acts on; warnings are surfaced but do not fail the gate on their own.
...
That distinction matters because the audit-trace check's citation pass is a textual `grep`: an ID string present in a test file counts, and the check cannot tell a real assertion from a fixture. So its coverage signal is textual — necessary but not sufficient. A repo whose own fixtures carry ID-shaped strings can name those files in the audit-trace ignore list in `docs/agents/project.md` so the pass skips them and does not report phantom citations.
...
**Changelog**, assembled by `cut-release` from that trailer plus the requirement's own text:
```markdown
### Shipped behavior
...
- [`specify-behavior`](../skills/specify-behavior.md) — where IDs are minted

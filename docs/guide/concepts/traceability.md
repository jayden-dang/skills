...
This is why [`frame-change`](../skills/frame-change.md) refuses to exit into code, and why [`specify-behavior`](../skills/specify-behavior.md) presents *the file* for approval rather than accepting conversational agreement:
...
Every link is a citation. Every citation is checkable by a fixed rule. And the check runs on every `prove-claim`, every `cut-release`, every `realign-spec`, and inside `plan-tasks`'s coverage check.
...
So the system makes a different bet: **the audit-trace is enforced by a mechanical check or it does not exist.**

The [audit-trace check](../resources/scripts.md#the-trace-check) is a fixed sequence of `grep` and `git` passes with a fixed rule on their output — nothing to install into the repo, no program and no interpreter. It reads the requirements files for definitions and the task footers and test files for citations, and it reports:
...
Determinism comes from the primitives: `grep` and `git` produce the same output every run, and the rule applied to that output leaves nothing to interpretation. This is [`author-skills`](../skills/author-skills.md)' doctrine applied to itself — a check that must never be skipped or misjudged is expressed as a set sequence of deterministic passes the skill runs and acts on, not as prose steps describing a check, because the output of a primitive is deterministic and language interpretation is not.
...
With it: `requirements.md` says `Status: Implemented`, `SHELL-1.2` has zero covering tests, and the audit-trace check reports `E2 SHELL-1.2 (docs/specs/…/requirements.md, Implemented) has no covering test`. [`prove-claim`](../skills/prove-claim.md) refuses to say "requirements met". [`cut-release`](../skills/cut-release.md) refuses to tag. The gap surfaces at the moment someone tries to claim the work is done.

## Its limits

The spine proves that a *test citing an ID exists*. It cannot prove the test ran, passed, or is a good test.

That is the job of the surrounding skills, and it is worth being clear that they are doing separate work: [`prove-claim`](../skills/prove-claim.md) demands each criterion checked against observed behavior; [`test-first`](../skills/test-first.md)'s anti-pattern table (tautological tests, tests that assert on mocks, tests coupled to implementation); `inspect-change`'s Standards axis; and the acceptance family driving the running system as a real client. A tautological test tagged `@SHELL-1.2` satisfies the audit-trace check completely and proves nothing at all.
...
- [The audit-trace check](../resources/scripts.md#the-trace-check) — the grep/git passes and their rules

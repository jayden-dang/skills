...
User-invoked (`/cut-release`). Distinct from `land-branch`, which only integrates a branch.
...
| Gate | What it does |
|---|---|
| **a. Prove-claim** | every verify command, fresh, plus the audit-trace check **clean**. An implemented requirement with no covering test is an untraced requirement, and untraced requirements block a cut-release |
...
| **b. Range set** | partition INDEX `Status:` in `last-tag..HEAD`. `Approved`/`In-progress`/`Draft` **stops** the cut before a version or tag |
...
Gate **b** is where the whole system cashes out. Commits carry `Implements: SHELL-1.2` and `Guards: SHELL-1.3` trailers. `cut-release` groups by those IDs, looks up each requirement's text in `docs/specs/`, and writes:
...
## `realign-spec` — the anti-rot skill

> Specs that are not resynced after change become fiction, and fiction is worse than no spec.

Run it whenever a spec'd feature changes outside its plan: requirements changed mid-implementation, the implementation deviated, the feature just shipped, or the audit-trace check comes back dirty.
...
A small in-scope change to an already-shipped, spec'd feature. **A fast lane, not a gate bypass**: every path still exits through `test-first`.

Ground the change in the existing spec, then classify **out loud**:

- **Tier 0** — no behavior change (a recolor, an icon, a label, a copy edit); every existing acceptance criterion still reads true unchanged → `test-first`.
- **Tier 1** — changes or extends existing spec'd behavior, ≤ ~half a day → mini-spec via `specify-behavior` (amend-feature the requirement, add a `SHALL CONTINUE TO` guard) → `test-first`.
- **Genuinely new scope** → stop amending. Escalate to `frame-change`.
...
If the feature has no spec at all, `amend-feature` is the wrong skill — a brand-new capability is `frame-change`, a break is `root-cause`.

## `/publish-issues` — the outgoing capture

The mirror of `/triage`. Where triage moves work *in* from the tracker, `publish-issues` pushes work *out* onto it — turning the current conversation, an approved spec, or a rough idea into **tracer-bullet issues**, each declaring the issues that block it. It is the fast lane that skips the full requirements → design → plan triad; `plan-tasks` remains the heavyweight, traceable publish path for tier-2 features.
...
Every published issue opens with `> *This issue was drafted by AI with `publish-issues`.*` — which discloses AI authorship and is the marker that keeps `triage` from re-triaging its own agent-ready output.
...
- **Issues born agent-ready are out of scope.** Anything published from a plan (recognizable by a `Requirements covered:` section) or by `publish-issues` (recognizable by its AI-drafted marker line) already has a real spec. Running it through triage adds a second, weaker one.
...
## `/scan-architecture` — the periodic scan
...
## `/write-handoff` — when the session ends

Compact the conversation into a document a fresh agent can resume from. Written to the **OS temp directory**, never into the workspace — a write-handoff is session ephemera, not a project artifact.
...
Two rules: **reference, never duplicate** — cite specs, plans, ADRs, issues, and commits by path or hash, because copying content bloats the write-handoff and *forks the truth*. And **redact secrets**, because the document may become another agent's prompt.
...
- [Traceability](../concepts/traceability.md) — why `cut-release` can derive its own changelog
- [Ceremony tiers](../methodology/ceremony-tiers.md) — where `amend-feature` routes each change
- [`realign-spec`](../skills/realign-spec.md) — the skill that keeps specs from becoming fiction

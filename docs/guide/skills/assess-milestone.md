...
[`refresh-roadmap-status`](refresh-roadmap-status.md) answers whether a milestone's **structure** is sound: is every item bound, does every status agree, is anything dangling. It deliberately refuses to answer whether the milestone *worked* — that judgment depends on the reader, and a check whose result depends on the reader is not a check.

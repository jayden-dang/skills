...
**Step 3.** `prove-claim` runs `pnpm typecheck`, `pnpm lint`, `pnpm test` fresh — all green — and a dry commit proves the pre-commit hook fires.

**Step 4.** Hand off: "scaffold complete; run `/configure-repo` next. Install with `pnpm install`, prove-claim with `pnpm test`."

## Why it is written the way it is

The skill guards two failure modes. The first is scaffolding on top of a real project — irreversible collisions with existing config — which is why Step 0 is a hard gate that inspects rather than asks, resolves the target before judging it, and redirects to `configure-repo` rather than pushing ahead. The second is a skeleton that looks wired but is not, which is why the deliverable is capped at one *passing* example test and Step 3 refuses to hand off on an unproven check. The root-only vs subpackage split keeps the same skill correct in a monorepo, where inheriting the parent's toolchain is right and installing a competing root hook or CI pipeline is wrong. The clean division of labor — this skill owns the harness, `configure-repo` owns `docs/agents/` — keeps each wizard's completion criterion sharp.

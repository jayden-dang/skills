...
|---|---|
| **Bucket** | track |
| **Invocation** | `/check-roadmap` (user-invoked; `disable-model-invocation: true`) |
...
| **Calls** | nothing. Names [`sync-spec`](sync-spec.md), [`write-roadmap`](write-roadmap.md), [`trace`](trace.md) and the feature-flow skills in its output |
| **Called by** | nobody — it is user-invoked |

## The horizontal counterpart to `trace`

[`trace`](trace.md) is the **vertical** check: does one feature's requirements, tasks, and tests agree with each other? `check-roadmap` is the **horizontal** one: do the plan and the specs agree, and what follows from that?

The two never overlap. `trace` never reads `docs/roadmap/INDEX.md` or the vision's `Goals` section; `check-roadmap` never reads a `tasks.md` footer or a test annotation. They touch the same string in exactly one place — a feature's `Status:` — and neither writes it.
...
So this skill recomputes. It writes no file, updates no status, and edits no roadmap. There is nothing to go stale and nothing to reconcile. Repair belongs elsewhere: [`sync-spec`](sync-spec.md) realigns a drifted `Status:`, [`write-roadmap`](write-roadmap.md) fixes the roadmap.
...
It walks from "the roadmap isn't approved yet" (→ `write-roadmap`) through the feature flow for the first incomplete member of the first committed milestone — `brainstorm`, `write-requirements`, `write-design`, `write-plan`, `execute-plan` — and ends at either naming `/release` for you or reporting the roadmap complete.
...
- [`write-roadmap`](write-roadmap.md) — authors the intent this reads
- [`trace`](trace.md) — the vertical check beside it
- [`sync-spec`](sync-spec.md) — repairs the `Status` drift this reports as `R10`

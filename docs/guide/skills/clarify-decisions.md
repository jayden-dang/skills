...
The map (prompts, plans) is not the territory (codebase, runtime, users, history). Clarify Decisions shrinks that gap before wrong guesses get expensive:
...
The user wants annotation export. Rather than a truncated picker or a questionnaire dump, `clarify-decisions` walks architecture first:

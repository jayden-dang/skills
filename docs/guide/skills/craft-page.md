...
- **Medium-agnostic.** The built-in assumes the Artifact tool. `craft-page` also covers the plain `.html` file a skill hands over when artifact tooling is absent — which is why the no-webfont-CDN rule cites two failure modes, not one: a published artifact's CSP blocks font hosts, and a handed-over file gets opened offline.

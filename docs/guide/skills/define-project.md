...
| **Calls** | [`clarify-decisions`](clarify-decisions.md), [`define-domain`](define-domain.md) (ADR gate), [`review-invariants`](review-invariants.md) (validate mode) |
...
- **validate** — walk each doc against its template, run [`review-invariants`](review-invariants.md) across the feature `design.md` files, and run the [`audit-trace`](audit-trace.md) invariant integrity check.
...
- [`review-invariants`](review-invariants.md) — the advisory conformance check `define-project` runs in validate mode

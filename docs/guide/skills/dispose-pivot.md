...
|---|---|
| **Bucket** | project |
| **Invocation** | `/dispose-pivot` (user-invoked; `disable-model-invocation: true`) |
...
| **Calls** | names `/anchor-project` (update) after confirmation; never invokes user-invoked skills |
| **Called by** | nobody auto — offered by `anchor-project` update when a pivot collides with shipped reality; named by `reroute-plan` on Vision-level rewinds with collisions |
...
- Does not write `docs/product/vision.md` or `docs/architecture/**` —
  `/anchor-project` owns those files
...
- [`anchor-project`](anchor-project.md) — sole writer of the vision layer

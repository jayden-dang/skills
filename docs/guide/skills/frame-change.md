...
1. **Explore project context.** Read `CONTEXT.md` (adopt its vocabulary from here on). Query the capability catalog per [`catalog-query.md`](../../../skills/execution/load-subgraph/references/catalog-query.md) — selected domain/feature/OBS cards only; never paste the full INDEX. Read the **Project posture** in `docs/agents/project.md` when present and let it right-size the interview: a Run Spike / Research / Learning posture means do not spend questions on data migration, backward compatibility, or deprecation; a released / Scaling / Maintenance posture means weigh exactly those. For anything heavier, dispatch a scan subagent that writes a findings digest to `.skills/<CODE>/scan.md` and returns only that path; work from the digest, not raw files. Then run the overlap check (below). If `docs/agents/project.md` or these files are missing, say so, suggest [`configure-repo`](configure-repo.md), and continue with what you have.

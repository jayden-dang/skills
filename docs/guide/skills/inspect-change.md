...
| **Calls** | map-features reverse runner **read-only** on the pinned range (step 1b); **names** [`/map-features`](map-features.md) when pending OBS need dispose; two READ-ONLY subagents (Standards, Spec) dispatched in parallel; [`load-subgraph`](load-subgraph.md) for the duplication / neighbor check; [`hold-stage`](hold-stage.md) before the Spec walk |
...
Collect whatever documents how code here should be written: CLAUDE.md, lint and formatter configs, CONTRIBUTING-style docs, plus `CONTEXT.md` for the repo's canonical vocabulary. If `docs/agents/project.md` is missing, note it, suggest running [`configure-repo`](configure-repo.md), and proceed with what exists.

...
Before any option is offered, [`prove-claim`](prove-claim.md) runs every verify command from `docs/agents/project.md` — typecheck, lint, unit, e2e — **fresh**, and confirms the [`audit-trace`](audit-trace.md) check is clean. A branch must not merge with untraced requirements; this is the same gate [`cut-release`](cut-release.md) enforces. If no test command is discoverable, the skill asks the user for it and suggests [`configure-repo`](configure-repo.md).

If the branch has user-facing behavior that has not been driven through the running system, [`validate-feature`](validate-feature.md) runs before Merge or PR is even offered — green units prove assertions pass, not that the feature works.
...
Cleanup happens only for a merge or a discard, and only for a worktree whose path sits under `.isolate-workspace/` or `isolate-workspace/` — that provenance means this skill set created it. Anything else, including harness-owned workspaces, is not yours to remove; leave it, or use the platform's own workspace-exit mechanism. The removal is never run from inside the worktree being removed — `cd` to the main repo root first. After removal, `git worktree prune` clears stale metadata.

## Close the loop

On a merge or PR, the skill reminds the user — or does it directly, if the spec's tasks are all complete — to run [`realign-spec`](realign-spec.md) to update the feature's `Status:` line and audit-trace state.
...
A feature branch `feat/shell-module-persistence` is complete in a linked worktree at `.isolate-workspace/shell-module-persistence`. The skill runs [`prove-claim`](prove-claim.md): typecheck, lint, and the unit suite pass fresh, and [`audit-trace`](audit-trace.md) reports every `SHELL` requirement covered. The feature is user-facing, so [`validate-feature`](validate-feature.md) drives module persistence through the running app — it passes and leaves committed tests behind.
...
The skill `cd`s to the main repo root, runs `git checkout main && git pull && git merge feat/shell-module-persistence`, and re-runs the verify suite on the merged result. It passes. Only now does the skill remove `.isolate-workspace/shell-module-persistence` from the main root, `git worktree prune`, `git branch -d feat/shell-module-persistence`, and hand off to [`realign-spec`](realign-spec.md) to flip the feature's status.
...
`land-branch` sits at the point of highest consequence — the moment work is integrated or destroyed — so it is written to remove every place an agent could act on a guess. The verbatim menu exists because inferring the user's intent here is exactly the failure it guards against: the integration decision is the user's, not the agent's. The typed-`discard` requirement exists because destructive actions must not fire on the same loose affirmatives that carry every other conversation. And the ordering rules — prove-claim before offering, merge and re-verify before cleanup, cleanup only for known-provenance isolate-workspace — all serve one invariant: never lose committed work to a shortcut.

## See also

- [`prove-claim`](prove-claim.md) — the gate that must pass before any option is offered
- [`cut-release`](cut-release.md) — the user-invoked sibling that cuts a version, not a branch merge
- [Traceability](../concepts/traceability.md) — why an untraced requirement blocks the merge
- [`isolate-workspace`](isolate-workspace.md) — how the `.isolate-workspace/` provenance this skill checks gets created

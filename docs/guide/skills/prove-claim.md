...
| **Calls** | [`validate-feature`](validate-feature.md) or [`walk-product`](walk-product.md) (to prove "the feature works"), the [`audit-trace`](audit-trace.md) check (to prove "requirements met") |
| **Called by** | nearly every skill before a completion claim — [`test-first`](test-first.md), [`root-cause`](root-cause.md), [`build-continuous`](build-continuous.md), [`land-branch`](land-branch.md), [`write-handoff`](write-handoff.md), and more |
...
1. **IDENTIFY** — which command proves this exact claim? Commands come from `docs/agents/project.md`; if it is missing, say so, suggest `configure-repo`, and identify the command manually.
...
Each claim has a required proof and a set of things that are never sufficient for it. The last row is the strictest: "requirements met" needs the [`audit-trace`](audit-trace.md) check to pass with zero errors **and** every acceptance criterion checked off individually.
...
| "The feature works" | The affected flow driven through the running system and observed ([`validate-feature`](validate-feature.md), or [`walk-product`](walk-product.md) for a manual pass) | Green unit tests alone |
...
`prove-claim` is the smallest skill in the set and deliberately so — it is a single gate the whole system routes through before any success is spoken, which is why so many other skills list it as a required last step. Its failure mode is not ignorance but momentum: the agent knows the tests should pass, is tired, and wants the task over, so it claims from memory. Everything here — the four-step function, the "never sufficient" column, the rationalization table that answers each excuse by name — exists to convert that momentum into one concrete action: run the command, read the output, then and only then say it.

## See also

- [The gates](../concepts/gates.md) — how this Iron Law relates to the other three
- [`test-first`](test-first.md) — hands off to `prove-claim` before any completion claim
- [`validate-feature`](validate-feature.md) — what proves "the feature works"
- [Traceability](../concepts/traceability.md) — why "requirements met" needs the audit-trace check

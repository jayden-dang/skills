...
3. **[`isolate-workspace`](isolate-workspace.md)** → **[`build-continuous`](build-continuous.md)** — an isolated workspace, then task-by-task execution with ledgered progress.
...
- Wanting to try a finished feature by hand in the real app routes to [`walk-product`](walk-product.md).
...
> **User:** `/route-work` I've got a bug report that the export button downloads an empty CSV. Where do I start?

`route-work` reads the situation — a reported defect, not new behavior — and routes rather than executes. It names the entry point and the chain:
...
Because `root-cause` is model-invocable, `route-work` hands off by invoking it and letting it take over. Had the situation instead been "review the PR my teammate opened", `route-work` would have named `/triage` and told the user to run it, since a user-invoked skill cannot be auto-invoked.

## Why it is written the way it is

`route-work` is a pure router, so per [`author-skills`](author-skills.md) it is a recipe skill, not a gate: its baseline failure is producing routing advice of the wrong shape, not breaking a rule under pressure. That is why it carries no iron law, no rationalization table, and no red flags — those forms measurably backfire on a skill whose job is to lay out a chain. Instead it is a positive contract: what a route IS, its steps in order. The `disable-model-invocation: true` frontmatter is itself doctrine — a router the agent could auto-invoke would fire on every ambiguous turn and pre-empt the actual entry skills; keeping it user-invoked means the human decides when routing help is wanted.

## See also

- [Methodology overview](../methodology/overview.md) — the same idea-to-ship chain in full
- [Ceremony tiers](../methodology/ceremony-tiers.md) — the tier-0/1/2 split `route-work` routes on
- [`gate-session`](gate-session.md) — the gate that names `route-work` when the flow is unclear
- [`frame-change`](frame-change.md) — the default entry point `route-work` points most tasks to

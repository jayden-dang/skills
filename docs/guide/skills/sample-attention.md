...
|---|---|
| **Bucket** | review |
| **Invocation** | user-invoked (`/sample-attention`) |
...
| **Called by** | none — [`build-continuous`](build-continuous.md) names it once as an optional aside |
...
This is the part worth repeating. `sample-attention` blocks no merge, no PR,
...
`skills/review/sample-attention/references/signals.md`.

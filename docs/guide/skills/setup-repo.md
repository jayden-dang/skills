...
Seven decisions, walked strictly one at a time: a two-or-three-sentence explainer, a recommendation with a one-line reason, then a wait for the user's answer. The skill assumes the user has never seen these concepts and never dumps all sections at once. (An eighth, the optional project-docs layer, is offered last and defaults to No.)

...
Once `Status` is Approved, an ID never changes meaning and is never renumbered. To retire a requirement you strike it through: `~~**CODE-1.2**~~ superseded by CODE-1.4`. The [`audit-trace`](audit-trace.md) check treats a struck-through ID as undefined, so any test or task still citing it surfaces immediately as an E1 error rather than rotting silently. This is why the ID, not the prose, is the unit of reference across the whole system — see [Requirement IDs](../concepts/requirement-ids.md).

## Worked example

The feature is `SHELL`: a left icon rail for switching between the app's modules. This example carries across the [`design-solution`](design-solution.md) and [`plan-tasks`](plan-tasks.md) pages, so the three read as one story.
...
The skill hands off as a required sub-skill: [`design-solution`](design-solution.md) for tier-2 features, or [`test-first`](test-first.md) directly for tier-1 fixes, carrying the new IDs. It never proceeds on conversational agreement — the file must be Approved first. Which route it takes is the tier decision made back in [`frame-change`](frame-change.md); see [ceremony tiers](../methodology/ceremony-tiers.md) for what separates a tier-1 fix from a tier-2 feature.

## Why it is written the way it is

Requirements are the root of the trace spine. If an ID is ambiguous, untestable, or built on a false claim about the current code, the error does not stay contained — it propagates through design, plan, tests, and shipped behavior, and each hop makes it harder to see. So the skill front-loads the cost: register the code before writing, force each criterion into a single EARS form, guard by active search rather than by default, and prove-claim every code-claim with a fresh pair of eyes that has no stake in the framing. The approval gate is on the written file precisely because the file — not the chat — is what the rest of the system will read.
...
- [`design-solution`](design-solution.md) — the next step for tier-2 work
- [`frame-change`](frame-change.md) — the discovery skill that hands off to this one

...
|---|---|
| **Bucket** | track |
| **Invocation** | `/status-roadmap` (user-invoked; `disable-model-invocation: true`) |
...
[`audit-trace`](audit-trace.md) is the **vertical** check: does one feature's requirements, tasks, and tests agree with each other? `status-roadmap` is the **horizontal** one: do the plan and the specs agree, and what follows from that?

The two never overlap. `audit-trace` never reads `docs/roadmap/INDEX.md` or the vision's `Goals` section; `status-roadmap` never reads a `tasks.md` footer or a test annotation. They touch the same string in exactly one place — a feature's `Status:` — and neither writes it.

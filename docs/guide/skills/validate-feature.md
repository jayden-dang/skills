...
| **Calls** | [`validate-api`](validate-api.md), [`validate-ui`](validate-ui.md), [`walk-product`](walk-product.md), [`root-cause`](root-cause.md) (through its children) |
| **Called by** | [`build-continuous`](build-continuous.md), [`land-branch`](land-branch.md), [`prove-claim`](prove-claim.md) |
...
Its place in the chain is fixed: it runs **after [`inspect-change`](inspect-change.md), before [`land-branch`](land-branch.md)**. Code review judges the diff; acceptance judges the running feature. The gap it exists to catch is the one a green suite hides:
...
This is an orchestrator. It does little validation itself; it derives the work, splits it by surface, hands each slice to a child skill, and closes the loop. What sets it apart from a manual [`walk-product`](walk-product.md) pass is what it leaves behind: committed, ID-tagged tests, not a one-time walkthrough.
...
| Human-eyeball qualities — visuals, feel | hand off to [`walk-product`](walk-product.md) |
...
The unit suite is green and `inspect-change` has passed, so acceptance is the next gate.
...
The API rows dispatch to [`validate-api`](validate-api.md); the UI rows to [`validate-ui`](validate-ui.md). Both receive `.skills/note-acceptance.md` by path and write their observed results back. Suppose `validate-api` finds `NOTE-1.2` returns `201` with a blank title instead of `422` — a real defect the unit tests missed because they asserted the handler's return value, not the wire status. It fixes the root cause through `root-cause`, leaves a committed `@NOTE-1.2` regression test, and marks the ledger row green.
...
- [`walk-product`](walk-product.md) — the manual child, for judgment a test cannot make
- [`drive-walk`](drive-walk.md) — run an existing walk-product guide end to end in the browser

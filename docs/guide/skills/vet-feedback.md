...
| **Reads** | the review items; the actual codebase (to prove-claim each claim); `docs/agents/project.md` (test conventions, via `test-first`) |
| **Writes** | fixes, one at a time, each through `test-first` (tests + production code + commits) |
| **Calls** | [`test-first`](test-first.md) (REQUIRED sub-skill — every fix goes through it) |
| **Called by** | any flow where review feedback arrives — from the user, a [`inspect-change`](inspect-change.md) subagent, or an external reviewer |
...
3. **PROVE CLAIM** each claim against the actual codebase before accepting it. The reviewer says the function ignores errors? Open the function. The reviewer says a path is dead? Audit Trace the callers. *Done when each item is confirmed, refuted, or marked unverifiable.*
...
6. **IMPLEMENT** one item at a time, in priority order: blocking items (breakage, security) → simple corrections (typos, imports) → complex changes (logic, refactoring). Each fix is production code, so the REQUIRED sub-skill is [`test-first`](test-first.md). *Done when each fix has its own passing test run and no regressions.*
...
| Reviewer subagents | Their findings are claims like any other; prove-claim before fixing. |
...
Each fix is production code, so each routes through [`test-first`](test-first.md) and earns its own passing test run with no regressions before the next item starts.
...
| "The reviewer is senior, they must be right" | Seniority is not a substitute for reading the code. Prove Claim. |

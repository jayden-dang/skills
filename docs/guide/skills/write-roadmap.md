...
| **Calls** | nothing |
| **Called by** | [`brainstorm`](brainstorm.md), when its step-5 decomposition names two or more independent sub-features |
...
So the roadmap carries no progress column, no per-milestone status field, no change log of transitions, and no percent-complete. A second copy of status drifts from the first, and once it drifts nobody can tell which is lying. [`check-roadmap`](check-roadmap.md) derives the live picture on demand instead.
...
|---|---|---|
| `GOAL-N` | `docs/product/vision.md` | [`establish-project`](establish-project.md) |
| `MILE-N`, `ROAD-N` | `docs/roadmap/INDEX.md` | this skill |
| feature codes, `CODE-N.M` | `docs/specs/` | [`write-requirements`](write-requirements.md) |
...
The skill validates the seven structural rules `S1`–`S7` carried in the template's comment block — the same set [`check-roadmap`](check-roadmap.md) re-checks as finding `R11`. Any defect is reported and the roadmap does not reach the user for approval. When clean, the whole file is presented and the skill stops; `Status: Approved` is written only after an explicit approval.
...
Forward-dependency detection and surface-overlap consolidation were specified, tested, and **retired** — fresh baseline agents did both unprompted, so writing them in would have been no-op text. Forward dependencies are still caught structurally, by `S4` in the rule block and by `check-roadmap`'s `R11`. The `Surfaces:` slot stays, because it is the data that makes overlap visible; what went away is the instruction to reason about it.

## Optionality

The layer is optional and gates nothing. Asked to consult a roadmap that does not exist, the skill says so and offers to author one. The feature flow runs fully without it.

## See also

- [`check-roadmap`](check-roadmap.md) — derives where the plan stands; never writes
- [`establish-project`](establish-project.md) — the vision and `ARCH-N` spine above this layer
- [`brainstorm`](brainstorm.md) — hands its multi-subsystem decomposition here

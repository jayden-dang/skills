# Research — an agent-driven walk-product runner
...
(`walk-product`), touching `plugin.json`, the guide docs, the README/AGENTS.md
...
`walk-product` produces a guide **for a human to drive**. Nothing in the set drives
it. The three neighbours each cover a different axis:

| Skill | Who drives | Deliverable | Durable? |
|---|---|---|---|
| `walk-product` | a human, later | the checkable HTML guide | the guide persists; the run does not |
| `validate-ui` | Playwright, headless | committed, ID-tagged specs | yes — joins the verify suite |
| `validate-api` | curl/http | committed API tests | yes |
...
breaks through `root-cause`, re-drive, and finish with every case accounted for.

It is not a duplicate of `validate-ui`. `validate-ui` writes assets; this
executes a checklist and self-heals. It is closest in shape to `build-continuous` —
a ledger-driven loop with a review gate and a circuit breaker — but over cases
instead of tasks.

## 2. Split justification

`author-skills` permits a split in exactly two cases. Both apply:
...
2. **Hiding post-completion steps.** Folding the drive loop into `walk-product` puts
   forty browser interactions in front of the author while it is still scoping
   cases, which is exactly the pull `author-skills` says a split should hide.

Verdict: a new skill, not a mode on `walk-product`.

## 3. Skill identity

**Name (recommended):** `drive-walk` — verb-first, kebab-case, carries the
literal keyword a user types (`walk-product`).

Alternatives considered: `run-walk-product` (collides with `/run`, which launches an
app), `execute-guide` (loses the `walk-product` keyword), `walk-cases` (coined, no
...
**Bucket:** `skills/acceptance/drive-walk/`.
**Invocation:** model-invocable (no `disable-model-invocation`).

**Draft description** — trigger + outcome noun only, no workflow steps:

> Use when a walk-product test guide already exists and its cases must be executed
...
> guide, working a walk-product file end to end, or testing every case in the browser
> and fixing what breaks. Not for authoring the guide (`walk-product`) or writing
> committed e2e specs (`validate-ui`).

The outcome noun is *the run ledger with a per-case verdict*. Naming the
neighbours inline follows the precedent in `plan-milestones` and `amend-feature`.

**Highest routing risk:** `walk-product` and `drive-walk` share a token, so the
...
| F3 | On a failure, patches inline without `root-cause` and without a failing test first | time + sunk cost + authority ("just make it green") | hard prohibition + rationalization table (this is a gate: Iron Laws 2 and 3) |
...
| F5 | Loses the run to compaction; restarts at case 1 or claims completion from memory | context pressure | ledger-as-source-of-truth, mirroring `build-continuous`'s `progress.md` |
...
F3 is the load-bearing one: the user's request ("nếu có lỗi, fix qua root-cause rồi
...
`.skills/<slug>-walk-product-run.md`, one row per case ID from the guide, written
...
| `notes` | fix attempts, the `root-cause` hand-off, the re-drive |
...
visible to the user (the `validate-feature` convention).

### 5.3 Failure routing (addresses F3/F4)

Triage before any fix — re-drive the failed case once from a clean state and
classify by an observable, the shape `validate-ui` already proves:
...
- **Deterministic failure on a real Expect** → product defect. `REQUIRED
  ...
  `root-cause` Phase 1 has something to build from.
- **Non-deterministic, or the guide is wrong** (a stale label, a missing
  ...
  guide, re-drive. Never route a guide bug to `root-cause`.
...
After `root-cause` reports fixed: restart the app, re-drive the failed case from a
...
run and escalates, matching Gate 3 and `build-continuous`'s breaker.
...
## 6. The `walk-product` upgrade

`walk-product` already produces everything the driver needs — for a human. Four
...
3. `data-backend="…"` — **the server-side assertion for this case**, or the
   ...
   too — a human product walker also cannot prove-claim persistence from the screen.
...
Form check: this is `author-skills`'s "omits an element from something it
...
`author-skills` rationalization table — baseline the current `walk-product` against a
...
| Role | Artifact |
|---|---|
| Source of truth during the run | `.skills/<slug>-walk-product-run.md` (verdict per case) |
| Session glanceable progress | one todo per case, marked done when the ledger row is `pass` |
| End-of-run write-handoff | regenerate or append a run report (statuses + `saw`/`server` evidence baked in) next to the guide path |
...
- On a deterministic product failure: stop that case → `root-cause` → re-drive failed
...
- **Per-case circuit breaker:** 3 distinct fix attempts on the same case → stop
  and escalate (Gate 3 / `build-continuous` precedent).
- **Per-run cap:** 5 distinct product-defect fix cycles across the whole run.
  ...
  must not turn a walk-product pass into an unbounded root-cause session.
...
- Passing cases: evidence lives in the run ledger / end report only. They do
  **not** auto-promote to Playwright/API specs (that is `validate-ui` /
  `validate-api` territory).
...
- Optional later (out of v1 scope): user may explicitly route-work to promote a green
  case into a committed e2e — that is a hand-off *to* `validate-ui`, not a
  silent side effect of `drive-walk`.

## 8. Ship checklist for this repo

Beyond the `author-skills` deployment checklist:

- [x] `skills/acceptance/drive-walk/SKILL.md` (≤300 lines preferred)
- [x] `walk-product/SKILL.md` edited with the four REQUIRED slots + the file-path rule
- [x] `.claude-plugin/plugin.json` — add `./skills/acceptance/drive-walk`
- [x] `docs/guide/skills/drive-walk.md` + `See also` links added to
      `walk-product.md`, `validate-ui.md`, `validate-feature.md`
...
- [x] `tests/drive-walk/red-baselines.md` + `scenarios-*.md`
- [x] `tests/trigger/drive-walk-routing.md` — query set for the colliding triple
      (live multi-model routing still outstanding)

## 9. RED outcome → GREEN scope (2026-07-26)

Full transcripts: `tests/drive-walk/red-baselines.md`. Model: **grok-4.5**
only (multi-model roster not available on this harness).

| Prediction | Observed | GREEN action |
|---|---|---|
| F1 screen-only tick | **RED 2/2** | Iron Law + REQUIRED `saw`/`server` slots; counter *"walk-product = screen only"* |
| F2 spot-check | **RED 1/2** (variance) | Ledger recipe binds full set; counter *"same pattern"* |
| F3 skip root-cause | **COMPLY 2/2** | Thin `REQUIRED SUB-SKILL: use root-cause` only — no Gate-2/3 clone |
...
| walk-product machine slots | **RED** slots 1–4 missing | REQUIRED `data-case/req/backend/setup` |

### Next

- [x] Research + skill identity + proposed body sketch
- [x] Open decisions closed (§7)
- [x] RED control runs + walk-product-upgrade RED
- [x] GREEN skill text for RED rows (+ D1–D3 recipe contracts)
- [x] GREEN re-run P-F7 + T-F1 + T-F2 + walk-product slots (grok-4.5 hold)
- [x] Ship checklist (§8): plugin, marketplace, guide docs, AGENTS/README counts, CHANGELOG, trigger query set
- [x] Follow-on 0.2.4: cases YAML + shell + CLI ledger (see `docs/specs/2026-07-27-walk-product-cli/contract.md`); P-F8/P-F9 RED→GREEN

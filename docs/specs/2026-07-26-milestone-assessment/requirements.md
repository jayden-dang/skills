...
**Namespaces this feature consumes.** `MILE-N` and `ROAD-N` (owned by `write-roadmap`),
`GOAL-N` (owned by `establish-project` via `vision.md`), feature codes and `CODE-N.M`
(owned by `write-requirements`). It introduces no new ID namespace: an assessment is
...
only by `write-roadmap`. `docs/roadmap/assessments/<MILE-N>.md` owns outcome truth and is
...
- **ASSESS-1.12** IF a structural rule that withholds `check-roadmap`'s next-action recommendation fires for the resolved milestone THEN THE SYSTEM SHALL report that finding and withhold the outcome verdict.
...
- ~~**ASSESS-3.4**~~ retired 2026-07-26: false premise. It read "WHERE an allocation … exists", implying a discoverable artifact, but `allocate-attention` persists none by default — `skills/review/allocate-attention/SKILL.md:38` ends its run with "no file exists unless they asked for one". Superseded by ASSESS-3.11.
...
- **ASSESS-3.8** WHEN the assessment produces a finding that requires follow-on work THE SYSTEM SHALL record that finding together with exactly one named destination among `amend`, `correct-course`, `write-roadmap`, `domain-modeling`, and `/file-issues`.
...
- **ASSESS-3.11** WHERE the user supplies an `allocate-attention` allocation covering the commit range from the milestone's committed baseline to its candidate closing revision THE SYSTEM SHALL count that allocation's sample set as sampled and carry its residue forward as explicitly unreviewed.
- **ASSESS-3.12** WHERE no such allocation is supplied THE SYSTEM SHALL record that range as unsampled and name `/allocate-attention` for the user to run.
...
- **ASSESS-4.2** WHEN evaluating mechanical eligibility THE SYSTEM SHALL require that the handoff names the same `MILE-N` and the same candidate closing revision as the assessment, that every member binding resolved, and that the committed baseline resolved.
...
- **ASSESS-4.5** WHEN close eligibility holds THE SYSTEM SHALL hand `write-roadmap` the `MILE-N`, the assessment's ordinal within that milestone's file, the effective verdict, and the candidate closing revision SHA.
- **ASSESS-4.6** WHEN `write-roadmap` receives an assessment handoff THE SYSTEM SHALL read the referenced assessment from `docs/roadmap/assessments/<MILE-N>.md` and verify that it names the same `MILE-N`, carries the referenced ordinal, records the same candidate closing revision SHA, holds a terminal current disposition, and carries the effective verdict and close decision the handoff asserts.
- **ASSESS-4.7** IF any value the handoff asserts does not match the referenced assessment THEN THE SYSTEM SHALL refuse the close and report the mismatch.
- **ASSESS-4.8** WHEN `write-roadmap` records a close from a verified assessment handoff THE SYSTEM SHALL write the verified SHA verbatim into that milestone's `Closed:` slot.
- **ASSESS-4.9** WHEN `write-roadmap` receives an assessment handoff THE SYSTEM SHALL neither re-run the assessment nor append a further assessment block.
...
- **ASSESS-4.12** IF `write-roadmap` is asked to transition a milestone from `Committed` to `Closed` without a verified assessment handoff THEN THE SYSTEM SHALL refuse the transition and name `/assess-milestone` for the user to run.
- **ASSESS-4.13** WHEN `write-roadmap` verifies an assessment handoff THE SYSTEM SHALL derive every verified value by reading the assessment artifact, treating the handoff's asserted values as claims to check rather than facts to trust.
...
- **ASSESS-5.2** WHEN a `Committed` milestone's members are all bound and every bound feature's `Status:` is `Shipped` THE SYSTEM SHALL select running `/assess-milestone` for that `MILE-N` as `check-roadmap`'s next action.
- **ASSESS-5.3** THE SYSTEM SHALL state the roadmap structural rules `R1`–`R11` in exactly one shared reference that both `check-roadmap` and `assess-milestone` read.
- **ASSESS-5.4** (guard) WHEN `check-roadmap` runs against a given repo state THE SYSTEM SHALL CONTINUE TO produce the finding set it produced before those rules moved to the shared reference.
- **ASSESS-5.5** (guard) WHEN `check-roadmap` runs THE SYSTEM SHALL CONTINUE TO write no file.
- **ASSESS-5.6** (guard) WHEN `check-roadmap` runs THE SYSTEM SHALL CONTINUE TO report structural presence only, judging no milestone outcome.
- **ASSESS-5.7** (guard) WHEN `write-roadmap` applies an update that does not transition a milestone to `Closed` THE SYSTEM SHALL CONTINUE TO apply its RMAP-1.17 approval gate unchanged.
- **ASSESS-5.8** (guard) WHEN `write-roadmap` runs THE SYSTEM SHALL CONTINUE TO leave `docs/specs/INDEX.md` unmodified.
- **ASSESS-5.9** (guard) WHEN `assess-milestone` runs THE SYSTEM SHALL CONTINUE TO leave `docs/roadmap/INDEX.md` modified only through `write-roadmap`.
- **ASSESS-5.10** (guard) WHEN `assess-milestone` needs an attention allocation THE SYSTEM SHALL CONTINUE TO leave `allocate-attention` user-invoked, naming it rather than invoking it.
- **ASSESS-5.11** (guard) WHEN `trace` runs THE SYSTEM SHALL CONTINUE TO check referential integrity for `CODE-N.M` and `ARCH-N` only.
- **ASSESS-5.12** (guard) WHEN a terminal human verdict is published THE SYSTEM SHALL CONTINUE TO restrict `record-decision`'s caller set to `finish-branch` and `release`.
- **ASSESS-5.13** WHEN `write-roadmap` records a close from a verified assessment handoff THE SYSTEM SHALL apply its RMAP-1.17 approval gate after the assessment gate has passed.
...
- **Assessing anything other than a milestone.** No feature-level and no arbitrary-range
  mode. `verify`, `code-review`, `acceptance-check`, and `sync-spec` already own the
...
- **Judging structural roadmap health.** `R1`–`R11` stay `check-roadmap`'s; this feature
...
- **`record-decision` as a destination.** Its caller set stays closed to `finish-branch`
  and `release` (ASSESS-5.12).
- **Estimation, velocity, capacity, forecasting, and next-milestone sizing.** ASSESS-3.6
  ...
  them, and nothing from this feature reaches `write-roadmap`'s planning path.
...
- **Reopening a closed milestone.** Moving a milestone's `Commitment:` off `Closed` is
  `write-roadmap`'s act under RMAP-1.19. This feature only appends a further assessment
  when asked to reassess.
- **Invoking `allocate-attention` or `check-roadmap`.** Both carry
...
- **Editing RMAP's Out-of-Scope section.** The reconciling note described in the preamble
  is `sync-spec`'s work against the RMAP triad, not a task of this feature.

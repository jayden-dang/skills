# Research — an agent-driven review-product-flow runner
...
(`review-product-flow`), touching `plugin.json`, the guide docs, the README/AGENTS.md
...
`review-product-flow` produces a guide **for a human to drive**. Nothing in the set drives
it. The three neighbours each cover a different axis:

| Skill | Who drives | Deliverable | Durable? |
|---|---|---|---|
| `review-product-flow` | a human, later | the checkable HTML guide | the guide persists; the run does not |
...
executes a checklist and self-heals. It is closest in shape to `build-in-waves` —
...
2. **Hiding post-completion steps.** Folding the drive loop into `review-product-flow` puts
...
Verdict: a new skill, not a mode on `review-product-flow`.

## 3. Skill identity

**Name (recommended):** `run-product-walkthrough` — verb-first, kebab-case, carries the
literal keyword a user types (`review-product-flow`).

Alternatives considered: `run-review-product-flow` (collides with `/run`, which launches an
app), `execute-guide` (loses the `review-product-flow` keyword), `walk-cases` (coined, no
...
**Bucket:** `skills/acceptance/run-product-walkthrough/`.
...
**Highest routing risk:** `review-product-flow` and `run-product-walkthrough` share a token, so the
...
| F5 | Loses the run to compaction; restarts at case 1 or claims completion from memory | context pressure | ledger-as-source-of-truth, mirroring `build-in-waves`'s `progress.md` |
...
`.skills/<slug>-review-product-flow-run.md`, one row per case ID from the guide, written
...
run and escalates, matching Gate 3 and `build-in-waves`'s breaker.
...
## 6. The `review-product-flow` upgrade

`review-product-flow` already produces everything the driver needs — for a human. Four
...
`author-skills` rationalization table — baseline the current `review-product-flow` against a
...
| Role | Artifact |
|---|---|
| Source of truth during the run | `.skills/<slug>-review-product-flow-run.md` (verdict per case) |
...
- **Per-case circuit breaker:** 3 distinct fix attempts on the same case → stop
  and escalate (Gate 3 / `build-in-waves` precedent).
- **Per-run cap:** 5 distinct product-defect fix cycles across the whole run.
  ...
  must not turn a review-product-flow pass into an unbounded root-cause session.
...
- [x] `skills/acceptance/run-product-walkthrough/SKILL.md` (≤300 lines preferred)
- [x] `review-product-flow/SKILL.md` edited with the four REQUIRED slots + the file-path rule
- [x] `.claude-plugin/plugin.json` — add `./skills/acceptance/run-product-walkthrough`
- [x] `docs/guide/skills/run-product-walkthrough.md` + `See also` links added to
      `review-product-flow.md`, `validate-ui.md`, `validate-feature.md`
...
- [x] `tests/run-product-walkthrough/red-baselines.md` + `scenarios-*.md`
- [x] `tests/trigger/run-product-walkthrough-routing.md` — query set for the colliding triple
      (live multi-model routing still outstanding)

## 9. RED outcome → GREEN scope (2026-07-26)

Full transcripts: `tests/run-product-walkthrough/red-baselines.md`. Model: **grok-4.5**
only (multi-model roster not available on this harness).

| Prediction | Observed | GREEN action |
|---|---|---|
| F1 screen-only tick | **RED 2/2** | Iron Law + REQUIRED `saw`/`server` slots; counter *"review-product-flow = screen only"* |
...
| review-product-flow machine slots | **RED** slots 1–4 missing | REQUIRED `data-case/req/backend/setup` |

### Next

- [x] Research + skill identity + proposed body sketch
- [x] Open decisions closed (§7)
- [x] RED control runs + review-product-flow-upgrade RED
- [x] GREEN skill text for RED rows (+ D1–D3 recipe contracts)
- [x] GREEN re-run P-F7 + T-F1 + T-F2 + review-product-flow slots (grok-4.5 hold)
- [x] Ship checklist (§8): plugin, marketplace, guide docs, AGENTS/README counts, CHANGELOG, trigger query set
- [x] Follow-on 0.2.4: cases YAML + shell + CLI ledger (see `docs/specs/2026-07-27-review-product-flow-cli/contract.md`); P-F8/P-F9 RED→GREEN

# Contract — review-product-flow cases + CLI + shell (2026-07-27)
...
|---|---|---|
| Cases catalog | `.skills/<slug>-review-product-flow.cases.yaml` | **Authoring SSOT** — cases, taxonomy, backend slots |
| Human guide | `.skills/<slug>-review-product-flow.html` | Generated view from shell + cases (localStorage ticks optional for humans) |
| Run ledger | `.skills/<slug>-review-product-flow-run.md` | **Agent progress SSOT** — verdicts + evidence |
| End report | `.skills/<slug>-review-product-flow-report.md` | End-of-run write-handoff (optional CLI `report`) |
...
```markdown
# Review Product Flow run ledger
- source: `.skills/notes-review-product-flow.cases.yaml`
- slug: notes
- origin: http://localhost:5173
...
Executable: `skills/acceptance/review-product-flow/scripts/review-product-flow` (python3).
...
`skills/acceptance/review-product-flow/shell/guide.html` — theme-aware CSS/JS, progress + localStorage for humans, injects cases via `/* __CASES_JSON__ */` or a replaced `__CASES_JSON__` marker.

`craft-page` is **optional** craft on the shell, not the default path.

---

## 6. Skill deltas (summary)

**review-product-flow:** coverage gate + ground + boot unchanged; §4 becomes write cases YAML → `review-product-flow render` → hand paths. No bespoke CSS unless user asks.

**run-product-walkthrough:** §2 `review-product-flow init` (or trust existing ledger); §3 `review-product-flow show` + drive **product** only; `review-product-flow mark` after evidence; §5 `review-product-flow report`. Guide HTML checkboxes never required.

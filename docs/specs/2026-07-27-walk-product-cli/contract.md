# Contract — walk-product cases + CLI + shell (2026-07-27)
...
|---|---|---|
| Cases catalog | `.skills/<slug>-walk-product.cases.yaml` | **Authoring SSOT** — cases, taxonomy, backend slots |
| Human guide | `.skills/<slug>-walk-product.html` | Generated view from shell + cases (localStorage ticks optional for humans) |
| Run ledger | `.skills/<slug>-walk-product-run.md` | **Agent progress SSOT** — verdicts + evidence |
| End report | `.skills/<slug>-walk-product-report.md` | End-of-run write-handoff (optional CLI `report`) |
...
```markdown
# Walk Product run ledger
- source: `.skills/notes-walk-product.cases.yaml`
- slug: notes
- origin: http://localhost:5173
...
Executable: `skills/acceptance/walk-product/scripts/walk-product` (python3).
...
`skills/acceptance/walk-product/shell/guide.html` — theme-aware CSS/JS, progress + localStorage for humans, injects cases via `/* __CASES_JSON__ */` or a replaced `__CASES_JSON__` marker.
...
**walk-product:** coverage gate + ground + boot unchanged; §4 becomes write cases YAML → `walk-product render` → hand paths. No bespoke CSS unless user asks.

**drive-walk:** §2 `walk-product init` (or trust existing ledger); §3 `walk-product show` + drive **product** only; `walk-product mark` after evidence; §5 `walk-product report`. Guide HTML checkboxes never required.

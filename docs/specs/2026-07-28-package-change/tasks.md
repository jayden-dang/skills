...
| File | Responsibility |
|---|---|
| `skills/ship/package-change/SKILL.md` | Orchestrates the six authoring phases; carries the hard gates and rationalization table |
| `skills/ship/package-change/conventions.md` | Commit- and PR-convention resolution ladder, bounds, and labelling rules |
| `skills/ship/package-change/tickets.md` | Ticket-set resolution, completion classification, backend linkage syntax |
| `skills/ship/package-change/package-contract.md` | Package layout, `manifest.md` field list, stable-id derivation, digest rules |
| `docs/guide/skills/package-change.md` | Human-facing guide page for the skill |
| `tests/package-change/scenarios.md` | Greppable ID annotation layer for every behavioral criterion |
| `tests/package-change/scenarios-pressure.md` | Injected-instruction and planted-credential pressure scenarios |
| `tests/trigger/package-change-routing.md` | Description-routing baseline for the new skill |
...
| `.claude-plugin/plugin.json` | Add `./skills/ship/package-change` |
| `.claude-plugin/marketplace.json` | Add `./skills/ship/package-change` |
...
**Files:**
- Create: `skills/ship/package-change/SKILL.md`
- Create: `docs/guide/skills/package-change.md`
- Create: `tests/package-change/scenarios.md`
- Create: `tests/trigger/package-change-routing.md`
...
**Interfaces:**
- Consumes: nothing
- Produces: `skills/ship/package-change/SKILL.md` with frontmatter `name: package-change`, no `disable-model-invocation` key, and a `## Phases` list naming the six phases in order: `Resolve base`, `Resolve conventions`, `Gather context`, `Resolve tickets`, `Author commits`, `Write package`
...
Add `./skills/ship/package-change` to the skill arrays in both
`.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json`, beside
`./skills/ship/land-branch`.

In `AGENTS.md`: bump the header count from `49 skills` to `50 skills`; add
`package-change` to the **ship** row of the §11 quick-reference table as `(m)`; add it
to the model-invoked list in §3. Leave §1 and §9 untouched.

In `README.md` line 185, change the ship row to
`| ship | \`package-change\`, \`land-branch\`, \`release\` |`.

Create `docs/guide/skills/package-change.md` following the shape of
`docs/guide/skills/brief-team.md`.

Create `tests/package-change/scenarios.md` with the section headings for all eleven
stories and the `PCHG-11.13` token; later tasks append their IDs.

Create `tests/trigger/package-change-routing.md` carrying `PCHG-1.1` and the routing
baseline rows: uncommitted work ready to commit → routes here; "open a PR" on an
already-committed branch → routes here then `land-branch`; "merge this" → routes to
`land-branch` only.
...
**Files:**
- Modify: `skills/ship/package-change/SKILL.md` (the `Resolve base` phase)
...
`/configure-repo` pointer, and the memoize / re-ask-when-it-no-longer-resolves rules.
Append `PCHG-2.1` … `PCHG-2.10` to `tests/package-change/scenarios.md` under a
...
**Files:**
- Create: `skills/ship/package-change/conventions.md`
- Modify: `skills/ship/package-change/SKILL.md` (the `Resolve conventions` phase)
- Modify: `tests/package-change/scenarios.md`
...
**Files:**
- Modify: `skills/ship/package-change/SKILL.md` (the `Gather context` phase)
- Modify: `tests/package-change/scenarios.md`
- Create: `tests/package-change/scenarios-pressure.md`
- Test: `tests/test_prepare_change_contract.py`

**Reuse:** existing — loads `skills/review/brief-team/references/passive-data-safety.md` verbatim rather than restating or copying it (rung 2)
...
class PrepareChangeContext(unittest.TestCase):
    ...
    def test_PCHG_3_4_loads_passive_data_contract_by_path(self):
        ...
        self.assertIn(
            "skills/review/brief-team/references/passive-data-safety.md", self.text
        )
...
skills/review/brief-team/references/passive-data-safety.md and follow it exactly`,
...
**Files:**
- Create: `skills/ship/package-change/tickets.md`
- Modify: `skills/ship/package-change/SKILL.md` (the `Resolve tickets` phase)
- Modify: `tests/package-change/scenarios.md`
- Test: `tests/test_prepare_change_contract.py`
...
**Files:**
- Modify: `skills/ship/package-change/SKILL.md` (the `Author commits` phase)
- Modify: `tests/package-change/scenarios.md`
- Test: `tests/test_prepare_change_contract.py`
...
**Files:**
- Create: `skills/ship/package-change/package-contract.md`
- Modify: `skills/ship/package-change/SKILL.md` (the `Write package` phase)
- Modify: `tests/package-change/scenarios.md`
- Test: `tests/test_prepare_change_contract.py`
...
**Files:**
- Modify: `skills/ship/package-change/SKILL.md` (the advisory-map and findings rules)
- Modify: `skills/ship/package-change/package-contract.md` (the map's manifest shape)
- Modify: `tests/package-change/scenarios.md`
- Test: `tests/test_prepare_change_contract.py`
...
class PrepareChangeAdvisory(unittest.TestCase):
    ...
    def test_PCHG_7_1_no_rewriting_verbs(self):
        ...
        advisory-commit-map section itself: `rewrite`, `amend-feature`, `squash`, and
        ...
        for verb in ("rewrite", "amend-feature", "squash", "reorder", "rebase", "force-push"):
...
check, and routing through the existing `prove-claim` failure path — adding no new gate —
...
### Task 9: The `land-branch` checkpoint

**Files:**
- Modify: `skills/ship/land-branch/SKILL.md` (Step 4, before the crossing)
- Modify: `tests/package-change/scenarios.md`
- Test: `tests/test_prepare_change_checkpoint.py`

**Reuse:** existing — extends `land-branch`'s Step 4 execution path; adds no menu item and no new gate (rung 2)
...
to run `/publish-issues`, named and never invoked; (2) on the PR path, display the exact
...
**Interfaces:**
- Consumes: the skill name `package-change` (Task 1)
- Produces: the closing sequence `review → fixer → polish-diff → acceptance → package-change → land-branch`
...
### Task 11: `configure-repo` decision and template slot

**Files:**
- Modify: `skills/setup/configure-repo/SKILL.md` (Step 2 decision list; Step 4 write list)
...
a two-or-three-sentence explainer naming `package-change` and `land-branch` as the

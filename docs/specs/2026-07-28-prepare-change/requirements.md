...
"The system" below means the `prepare-change` skill unless a criterion names
another skill explicitly (`finish-branch`, `execute-plan`, `setup-repo`).
-->
...
- **PCHG-2.8** WHEN `Default PR base:` or `docs/agents/project.md` is absent THE SYSTEM SHALL proceed with the base the user gave for this invocation and SHALL name `/setup-repo` as the way to persist it.
...
- **PCHG-7.6** WHERE a machine-enforced convention check ran and failed THE SYSTEM SHALL route the failure through the repository's existing `verify` failure path and SHALL NOT introduce an additional gate for it.
...
- **PCHG-8.1** WHEN `finish-branch` reaches a merge or PR crossing THE SYSTEM SHALL display the resolved ticket set and ask whether missing tickets should be created or supplemented, before the crossing executes.
- **PCHG-8.2** WHEN no tracker is configured THE SYSTEM SHALL still ask the missing-ticket question at that checkpoint.
- **PCHG-8.3** IF the user asks for a missing ticket to be created THEN `finish-branch` SHALL pause the crossing and ask the user to run `/file-issues`, and SHALL NOT invoke that skill itself.
...
- **PCHG-9.1** WHEN `execute-plan` has completed its whole-branch review, polish, and acceptance steps THE SYSTEM SHALL run before `finish-branch`.
...
- **PCHG-9.4** WHEN running as the continuation of `execute-plan` THE SYSTEM SHALL create commits without a further approval step, asking only the exception questions in PCHG-1.5.
...
- **PCHG-10.1** WHEN `setup-repo` walks its configuration decisions THE SYSTEM SHALL ask the user to choose a default PR base.
...
|---|---|
| `skills/ship/prepare-change/SKILL.md` | new file — no behavior to guard |
| `skills/ship/finish-branch/SKILL.md` | verify/trace/acceptance gate, five-option menu, record-before-crossing, discard confirmation, worktree provenance, risk-glob naming, no-force-push |
| `skills/execution/execute-plan/SKILL.md` | After-the-Last-Task ordering, ledger writes, continuous-mode no-pause rule |
| `skills/setup/setup-repo/SKILL.md` | one-decision-at-a-time walk, additive write rule, Step 6 verification gate |
...
| `docs/guide/skills/prepare-change.md` | new file — no behavior to guard |
...
| `docs/roadmap/INDEX.md` | already updated under `write-roadmap`; no behavior to guard |

- **PCHG-11.1** (guard) WHEN a branch reaches `finish-branch` THE SYSTEM SHALL CONTINUE TO withhold the merge and PR options while any verify command, the trace check, or a required acceptance check fails.
- **PCHG-11.2** (guard) WHEN `finish-branch` presents its menu on a green gate THE SYSTEM SHALL CONTINUE TO present its five options verbatim, with the new checkpoint occurring after the user's selection rather than as an additional menu item.
- **PCHG-11.3** (guard) WHEN `finish-branch` executes a merge, PR, discard, or block THE SYSTEM SHALL CONTINUE TO publish a validator-clean record through `record-decision` before the crossing runs.
- **PCHG-11.4** (guard) WHEN `finish-branch` executes a discard THE SYSTEM SHALL CONTINUE TO require the user to type the word `discard`.
- **PCHG-11.5** (guard) WHEN `finish-branch` closes the loop THE SYSTEM SHALL CONTINUE TO name `/comprehend-change` and `/explain-change` under its existing risk-glob predicates.
...
- **PCHG-11.7** (guard) WHEN `execute-plan` finishes its last task THE SYSTEM SHALL CONTINUE TO run whole-branch review, then one fixer pass if needed, then polish, then acceptance, in that order, before any ship step.
- **PCHG-11.8** (guard) WHEN `execute-plan` runs in continuous mode THE SYSTEM SHALL CONTINUE TO avoid pausing between tasks to ask permission to continue.
- **PCHG-11.9** (guard) WHEN `execute-plan` completes a task THE SYSTEM SHALL CONTINUE TO append that task's line to `.skills/progress.md`.
- **PCHG-11.10** (guard) WHEN `setup-repo` gathers configuration THE SYSTEM SHALL CONTINUE TO walk its decisions one at a time, waiting for the user's answer before moving on.
- **PCHG-11.11** (guard) WHEN `setup-repo` writes a file that already exists THE SYSTEM SHALL CONTINUE TO merge additively and preserve content the user wrote.
- **PCHG-11.12** (guard) WHEN `setup-repo` finishes writing THE SYSTEM SHALL CONTINUE TO run its Step 6 verification gate over every configured command.
...
- Aligning `explain-change`'s range resolver with the declared PR base — deferred to `ROAD-5` (shared-base-resolution-for-explainers) under MILE-2.
- Any persistent convention cache across sessions — unscheduled until a measured cost justifies one.
- Pushing the branch, creating the pull request, merging, discarding, or blocking: those crossings remain `finish-branch`'s alone.
- Creating, editing, or closing tracker items: `prepare-change` reads and links them; `/file-issues` remains the only way work is filed.
- Running verification: `prepare-change` neither runs nor re-runs the verify suite, the trace check, or acceptance checks.

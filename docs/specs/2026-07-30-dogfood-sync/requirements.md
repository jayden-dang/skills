...
Today a dogfood run is spread across three files: the cases YAML the agent
...
**Story:** As an agent driving a dogfood run, I want to mark a case and have the
guide's own data file change, so that what I proved and what the person reads
are the same record.

- **DFSYNC-1.1** THE SYSTEM SHALL keep each dogfood run in exactly one file, `.skills/<slug>-dogfood.json`, holding both case content and run state.
- **DFSYNC-1.2** WHEN any `dogfood` subcommand needs case content and run state THE SYSTEM SHALL read both from a single path argument, with no separate catalog path or ledger path.
- **DFSYNC-1.3** WHEN `dogfood init` runs against a file whose cases carry no run state THE SYSTEM SHALL add `verdict: pending` to every case and leave any case that already has run state untouched.
- **DFSYNC-1.4** IF `dogfood init` runs against a file that already holds a non-`pending` verdict and `--force` is absent THEN THE SYSTEM SHALL exit non-zero and write nothing.
...
- **DFSYNC-1.6** THE SYSTEM SHALL execute every `dogfood` subcommand using only the Python standard library.
...
- **DFSYNC-1.8** (guard) WHEN `dogfood list` runs THE SYSTEM SHALL CONTINUE TO print one tab-separated `id`, `req`, `kind`, `title` line per case, in file order.
- **DFSYNC-1.9** (guard) WHEN `dogfood show` runs against a case THE SYSTEM SHALL CONTINUE TO print its `id`, `req`, `kind`, `title`, `setup`, `try`, `expect`, and `backend`.
- **DFSYNC-1.10** (guard) WHEN `dogfood next` finds no case whose verdict is other than `pass` THE SYSTEM SHALL CONTINUE TO print nothing and exit 1.
- **DFSYNC-1.11** (guard) WHEN `dogfood mark` is called with verdict `pass` THE SYSTEM SHALL CONTINUE TO reject an empty `--saw` and reject an empty `--server`.
...
- **DFSYNC-1.13** (guard) WHEN `dogfood report` writes a report THE SYSTEM SHALL CONTINUE TO emit a markdown table with one row per case and to escape `|` in cell text.
...
- **DFSYNC-2.3** WHEN `dogfood next` selects a case THE SYSTEM SHALL return the first case whose `verdict` is not `pass`, disregarding every `human` field.
- **DFSYNC-2.4** WHEN `dogfood status` runs THE SYSTEM SHALL print the count of human-ticked cases on its own line, separate from the verdict counts.
- **DFSYNC-2.5** WHEN `dogfood report` writes a report THE SYSTEM SHALL give the human tick its own column, distinct from the verdict column.
...
- **DFSYNC-3.5** THE SYSTEM SHALL complete `dogfood mark` without opening a network connection.
- **DFSYNC-3.6** WHILE a serve process is running THE SYSTEM SHALL leave the observable result of `dogfood mark` identical to its result when no serve process is running.

## 4. A guide that is correct on its own

**Story:** As a person handed a dogfood guide, I want to double-click the HTML
file and see which cases have passed, so that the guide is useful with nothing
else running.

- **DFSYNC-4.1** WHEN `dogfood render` runs THE SYSTEM SHALL embed both case content and each case's current verdict into the generated HTML.
...
- **DFSYNC-4.8** (guard) WHEN `dogfood render` embeds the payload THE SYSTEM SHALL CONTINUE TO preserve two-character `\n` sequences inside JSON strings instead of expanding them into real newlines.
...
- **DFSYNC-5.1** WHEN `dogfood serve` starts THE SYSTEM SHALL bind only the loopback interface `127.0.0.1`.
- **DFSYNC-5.2** WHEN `dogfood serve` starts THE SYSTEM SHALL bind port 8787, or the next free port above it when 8787 is in use, and print the URL it actually bound.
...
- **DFSYNC-5.4** WHILE the guide is open over HTTP THE SYSTEM SHALL show verdicts written by `dogfood mark` without the person reloading the page.
...
- **DFSYNC-5.6** WHEN `dogfood serve` starts THE SYSTEM SHALL write `.skills/<slug>-dogfood-serve.pid` holding the process id, the bound port, and a token unique to that server instance.
- **DFSYNC-5.7** WHEN the agent starts `dogfood serve` THE SYSTEM SHALL run it in the background and return control to the agent.
- **DFSYNC-5.8** WHERE no serve process is running THE SYSTEM SHALL leave every other `dogfood` subcommand fully usable.

## 6. Stopping the server without collateral damage

**Story:** As a person whose dogfood run has finished, I want to be asked whether
to shut the server down, so that it never lingers unnoticed and never takes an
unrelated process down with it.

- **DFSYNC-6.1** WHEN `dogfood serve --stop` runs THE SYSTEM SHALL request `/whoami` on the port recorded in the pidfile and terminate the recorded process only when the returned token equals the pidfile's token.
...
- **DFSYNC-6.3** WHEN `dogfood serve` starts and finds an existing pidfile THE SYSTEM SHALL apply the same token verification and remove a stale pidfile without terminating any process.
- **DFSYNC-6.4** WHEN a dogfood run reaches its end while a serve process the agent started is still running THE SYSTEM SHALL ask the person whether to stop it.
...
- **Performance:** **DFSYNC-7.1** WHEN `dogfood mark` writes a verdict while a guide is open over HTTP THE SYSTEM SHALL make that verdict visible on the page within 3 seconds — verified by an automated test that marks a case, then polls the served state until the verdict appears or the deadline passes.
...
|---|---|---|
| `skills/acceptance/dogfood/scripts/dogfood` | list/show/next output and exit codes; `validate_mark` evidence rules; report table escaping; catalog validation | DFSYNC-1.8, 1.9, 1.10, 1.11, 1.12, 1.13 |
| `skills/acceptance/dogfood/shell/guide.html` | HTML escaping; `data-*` attributes; localStorage ticks and reset; kind chips; light/dark scheme | DFSYNC-4.4, 4.5, 4.6, 4.9 |
| `skills/acceptance/dogfood/scripts/dogfood` (`render_html`) | marker check; `\n` preservation via callable replacement | DFSYNC-4.7, 4.8 |
| `skills/acceptance/dogfood/SKILL.md` | coverage gate (§1 rules 1–5); seven-kind taxonomy; "never ship a chat-only checklist" | DFSYNC-1.5 carries the taxonomy; coverage gate and taxonomy prose are edited, not removed — no behavior to guard beyond DFSYNC-1.5 |
| `skills/acceptance/drive-dogfood/SKILL.md` | Iron Law evidence bar; origin consent gate; failure routing and caps | DFSYNC-1.11, 1.12 carry the evidence bar; origin gate and caps are untouched — no behavior to guard |
| `skills/acceptance/dogfood/references/cases-schema.md` | schema documentation only | no behavior to guard |
| `tests/test_dogfood_cli.py` | test file — rewritten against the new format | no behavior to guard |
| `tests/drive-dogfood/fixtures/notes-app/notes-dogfood.cases.yaml` | fixture — converted to JSON | no behavior to guard |
| `tests/drive-dogfood/fixtures/notes-app/dogfood-guide.html` | fixture — regenerated | no behavior to guard |
| `tests/drive-dogfood/scenarios-cli.md` | scenario prose citing ledger paths | no behavior to guard |
| `tests/drive-dogfood/red-baselines.md` | recorded RED transcripts — historical record, not edited | no behavior to guard |
| `docs/agents/project.md` | trace-ignore list naming dogfood test paths | no behavior to guard |
| `docs/guide/skills/dogfood.md`, `docs/guide/skills/drive-dogfood.md` | human documentation | no behavior to guard |
...
- **Migration from v1.** No `dogfood migrate` command, no reader for `.cases.yaml` or `-run.md`. Existing artifacts are deleted by hand. `.skills/` is git-ignored, so no committed artifact is at risk.
...
- **Reading the guide HTML as a catalog fallback.** Only `extract_dogfood_json` survives; the regex scraper over `data-*` attributes is removed.
- **Promoting passing cases into committed e2e specs.** Unchanged — that remains `acceptance-ui` territory, reached only when the user asks.

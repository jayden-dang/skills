# Tasks: Review Product Flow sync
...
**Goal:** Collapse a review-product-flow run's three disconnected files into one JSON artifact
...
**Architecture:** `.skills/<slug>-review-product-flow.json` (`version: 2`) holds authored case
...
field-scoped patch, bumps `rev`, and lands via `os.replace`. `review-product-flow render` bakes
current verdicts into the guide so a `file://` page is correct with nothing running;
`review-product-flow serve` adds a loopback-only HTTP layer on top, whose shutdown terminates a
...
`audit-trace` grep. **Every test in this plan opens with `"""DFSYNC-N.M — …"""`.**
...
| Path | Responsibility |
|---|---|
| `tests/run-product-walkthrough/fixtures/notes-app/notes-review-product-flow.json` | The six-case notes fixture, converted to v2 with run state |
| `tests/test_walk_product_store.py` | The one new seam — locked, atomic, `rev`-checked commit |
| `tests/test_walk_product_serve.py` | Serve endpoints, bind address, pidfile, verified shutdown |
| `tests/test_walk_product_guide_contract.py` | Static contract assertions over `shell/guide.html` |

**Modify:**

| Path | Responsibility |
|---|---|
| `skills/acceptance/review-product-flow/scripts/review-product-flow` | v2 schema, store, command surface, `serve` |
| `skills/acceptance/review-product-flow/shell/guide.html` | Verdict badges, dual `file://` / `http://` mode |
| `skills/acceptance/review-product-flow/SKILL.md` | Authoring flow for v2; `serve` in the hand-over |
| `skills/acceptance/review-product-flow/references/cases-schema.md` | v2 schema reference |
| `skills/acceptance/run-product-walkthrough/SKILL.md` | Iron Law line, two rationalization rows, CLI recipe, ask-to-stop |
| `tests/test_walk_product_cli.py` | Rewritten against v2 |
| `tests/run-product-walkthrough/scenarios-cli.md` | CLI recipes updated to v2 paths |
| `docs/agents/project.md` | Audit Trace-ignore paths; `Browser E2E` row |
| `docs/guide/skills/review-product-flow.md`, `docs/guide/skills/run-product-walkthrough.md` | Human docs |
| `CHANGELOG.md` | Cut Release note |

**Delete:**

| Path | Reason |
|---|---|
| `tests/run-product-walkthrough/fixtures/notes-app/notes-review-product-flow.cases.yaml` | Superseded by the `.json` fixture |
| `tests/run-product-walkthrough/fixtures/notes-app/review-product-flow-guide.html` | Regenerated from the v2 fixture |
...
**Files:**
- Modify: `skills/acceptance/review-product-flow/scripts/review-product-flow` — delete `load_cases_yaml` (:53-94), `load_cases_html` (:197-272), `extract_label_block` (:275-291), `extract_title` (:294-300), `extract_req` (:303-308), `strip_tags` (:311-315), `collapse_ws` (:318-320), and the `try: import yaml` block (:18-21); rewrite `load_catalog` (:37-50) and `catalog_from_dict` (:117-155)
- Create: `tests/run-product-walkthrough/fixtures/notes-app/notes-review-product-flow.json`
- Delete: `tests/run-product-walkthrough/fixtures/notes-app/notes-review-product-flow.cases.yaml`
- Test: `tests/test_walk_product_cli.py`

**Reuse:** existing — extends the eight-slot case shape already validated by `normalize_case` (`scripts/review-product-flow:97`) (rung 2)
...
```python
# tests/test_walk_product_cli.py — replacing the v1 fixture constant and adding these
RUN = REPO / "tests" / "run-product-walkthrough" / "fixtures" / "notes-app" / "notes-review-product-flow.json"
...
class SchemaV2Tests(unittest.TestCase):
    ...
    def test_v1_paths_are_named_errors_not_parse_attempts(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for name in ("old.cases.yaml", "old.yml", "notes-review-product-flow-run.md"):
                stale = Path(tmp) / name
...
Run: `python3 -m unittest tests.test_walk_product_cli` — expect: import/attribute errors and
...
Translate all six cases from `notes-review-product-flow.cases.yaml` to `notes-review-product-flow.json`
...
def load_run_file(path: Path) -> Dict[str, Any]:
    ...
    if path.suffix.lower() in {".yaml", ".yml"} or path.name.endswith("-run.md"):
        raise SystemExit(
            ...
            "run file (.skills/<slug>-review-product-flow.json). Delete the v1 files and re-author."
        )
    if not path.is_file():
...
`extract_walk_product_json` (:158-194) — it stays useful now that the embedded blob is
the whole document.

Run: `python3 -m unittest tests.test_walk_product_cli` — expect: pass.

- [ ] **Step 4: Commit**

`git commit -m "feat(review-product-flow): v2 JSON run file, drop YAML and HTML scrapers"`
...
**Files:**
- Modify: `skills/acceptance/review-product-flow/scripts/review-product-flow` — delete the ledger block `default_run_path` (:345-348), `init_ledger` (:351-375), `parse_ledger` (:378-413), `write_ledger` (:416-437) and the `HEADER_RE`/`FIELD_RE` constants (:341-342); add the store
- Create: `tests/test_walk_product_store.py`
...
```python
# tests/test_walk_product_store.py
...
Run: `python3 -m unittest tests.test_walk_product_store` — expect: `ImportError`/`NameError`
...
Run: `python3 -m unittest tests.test_walk_product_store` — expect: pass.

- [ ] **Step 3: Commit**

`git commit -m "feat(review-product-flow): locked, atomic, rev-checked store"`
...
**Files:**
- Modify: `skills/acceptance/review-product-flow/scripts/review-product-flow` — `cmd_init` (:543-551), `cmd_status` (:554-568), `cmd_next` (:571-577), `cmd_mark` (:580-618), `cmd_report` (:635-668), `build_parser` (:671-731)
- Test: `tests/test_walk_product_cli.py`

**Reuse:** existing — keeps the argparse subcommand layout (`scripts/review-product-flow:671`) and `validate_mark` unchanged (`:447`) (rung 2)
...
Fill each body following the existing style in `tests/test_walk_product_cli.py`.
...
does in Task 1. This is why `tests/test_walk_product_cli.py` can leave the trace-ignore
list in Task 8 — see *Audit Trace-ignore* under the coverage check.

Run: `python3 -m unittest tests.test_walk_product_cli` — expect: failures on the human
...
`git commit -m "feat(review-product-flow): verdict commands read and write the v2 run file"`
...
**Files:**
- Modify: `skills/acceptance/review-product-flow/scripts/review-product-flow` — `render_html` (:484-513), `cmd_render` (:621-632)
- Modify: `skills/acceptance/review-product-flow/shell/guide.html` — CSS (:8-137), `renderCase` (:230-257), the IIFE head (:172-208)
- Create: `tests/test_walk_product_guide_contract.py`
- Test: `tests/test_walk_product_cli.py`
- Delete: `tests/run-product-walkthrough/fixtures/notes-app/review-product-flow-guide.html` (regenerated)

**Reuse:** existing — extends the `__CASES_JSON__` marker protocol (`scripts/review-product-flow:484`) and the guide's render IIFE (`shell/guide.html:172`) (rung 2)

**Interfaces:**
- Consumes: `load_run_file` (Task 1)
- Produces: rendered HTML whose embedded `window.__WALK PRODUCT__` is the whole v2 document plus `"rendered_at"`; `window.__WALK PRODUCT__.rendered_at` is what the offline banner prints
...
```python
# tests/test_walk_product_cli.py
...
# tests/test_walk_product_guide_contract.py — static assertions over the shell source
...
Run: `python3 -m unittest tests.test_walk_product_cli tests.test_walk_product_guide_contract` —
...
`python3 skills/acceptance/review-product-flow/scripts/review-product-flow render tests/run-product-walkthrough/fixtures/notes-app/notes-review-product-flow.json -o tests/run-product-walkthrough/fixtures/notes-app/review-product-flow-guide.html`

- [ ] **Step 3: Commit**

`git commit -m "feat(review-product-flow): render bakes verdicts; guide declares its snapshot"`
...
### Task 5: `review-product-flow serve` — the loopback layer

**Files:**
- Modify: `skills/acceptance/review-product-flow/scripts/review-product-flow` — add `cmd_serve` and the handler; register the subparser in `build_parser`
- Create: `tests/test_walk_product_serve.py`
...
Run: `python3 -m unittest tests.test_walk_product_serve` — expect: `NameError: bind_server`.
...
Run: `python3 -m unittest tests.test_walk_product_serve` — expect: pass.

- [ ] **Step 3: Commit**

`git commit -m "feat(review-product-flow): loopback serve layer with human-scoped writes"`
...
**Files:**
- Modify: `skills/acceptance/review-product-flow/scripts/review-product-flow` — `cmd_serve` gains pidfile write and `--stop`
- Test: `tests/test_walk_product_serve.py`
...
Run: `python3 -m unittest tests.test_walk_product_serve` — expect: `NameError: pidfile_path`.

- [ ] **Step 2: Implement**

On start write `{"pid", "port", "token", "slug"}` to
`.skills/<slug>-review-product-flow-serve.pid` with `secrets.token_hex(16)`. `verify_live` GETs
`http://127.0.0.1:<port>/whoami` with a 1 s timeout and compares tokens. `--stop`:
prove-claim → `os.kill(pid, signal.SIGTERM)` and remove the pidfile on match; on no
...
`git commit -m "feat(review-product-flow): token-verified server shutdown"`
...
**Files:**
- Modify: `skills/acceptance/review-product-flow/shell/guide.html` — the IIFE gains the polling branch
- Test: `tests/test_walk_product_guide_contract.py`, `tests/test_walk_product_serve.py`
...
```python
# tests/test_walk_product_guide_contract.py
...
# tests/test_walk_product_serve.py
def test_served_page_carries_the_live_client(self):
    """DFSYNC-5.4 — the page served over http ships the polling client."""
```

Run: `python3 -m unittest tests.test_walk_product_guide_contract tests.test_walk_product_serve`
...
`git commit -m "feat(review-product-flow): live polling and tick POST in the served guide"`
...
**Files:**
- Modify: `skills/acceptance/run-product-walkthrough/SKILL.md` — Iron Law second line (:26), CLI recipe (:47-67), failure routing reference to cases YAML (:137), §5 close (:155-163), rationalization rows (:170-171), red flags (:180-185)
- Modify: `skills/acceptance/review-product-flow/SKILL.md` — §4 authoring (:82-113), §5 hand-over (:115-125), red flags (:141-145)
- Modify: `skills/acceptance/review-product-flow/references/cases-schema.md` — full rewrite for v2
- Modify: `tests/run-product-walkthrough/scenarios-cli.md`, `docs/agents/project.md`, `docs/guide/skills/review-product-flow.md`, `docs/guide/skills/run-product-walkthrough.md`, `CHANGELOG.md`
- Test: `tests/test_walk_product_cli.py` (skill-body contract assertions, matching the style of `tests/test_prepare_change_wiring.py`)

**Reuse:** existing — extends `run-product-walkthrough` §5 *Close the run* and `review-product-flow` §5 *Hand over* (rung 2)
...
**Audit Trace-ignore — do this exactly.** In the ignore list at
`docs/agents/project.md:112`, **remove** `tests/test_walk_product_cli.py` and **do not
add** `tests/test_walk_product_store.py`, `tests/test_walk_product_serve.py`, or
`tests/test_walk_product_guide_contract.py`. Keep `tests/run-product-walkthrough/fixtures/` and
`tests/run-product-walkthrough/scenarios-cli.md` ignored — those still carry fixture IDs as
data. Then prove it:
...
def test_dfsync_tests_are_visible_to_trace(self):
    ...
    for mod in ("test_walk_product_cli.py", "test_walk_product_store.py",
                "test_walk_product_serve.py", "test_walk_product_guide_contract.py"):
...
`git commit -m "docs(review-product-flow): skill bodies and references for the v2 run file"`
...
### Audit Trace-ignore — the landmine this plan defuses

`tests/test_walk_product_cli.py` is currently on the trace-ignore list
(`docs/agents/project.md:112`), and `audit-trace`'s NON-NEGOTIABLE section drops ignored
...
(`tests/test_walk_product_guide_contract.py`), in the same style as the repo's existing
...
5.5 are covered behaviorally in `tests/test_walk_product_serve.py`.
...
browser. That behavioral half belongs to `validate-ui`, which owns harness setup,
...
behavior to `validate-ui` later — no Playwright harness in this branch. Tracker

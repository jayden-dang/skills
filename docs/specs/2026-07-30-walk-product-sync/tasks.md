...
**Goal:** Collapse a walk-product run's three disconnected files into one JSON artifact
...
**Architecture:** `.skills/<slug>-walk-product.json` (`version: 2`) holds authored case
...
field-scoped patch, bumps `rev`, and lands via `os.replace`. `walk-product render` bakes
current verdicts into the guide so a `file://` page is correct with nothing running;
`walk-product serve` adds a loopback-only HTTP layer on top, whose shutdown terminates a
...
| Path | Responsibility |
|---|---|
| `tests/drive-walk/fixtures/notes-app/notes-walk-product.json` | The six-case notes fixture, converted to v2 with run state |
...
| Path | Responsibility |
|---|---|
| `skills/acceptance/walk-product/scripts/walk-product` | v2 schema, store, command surface, `serve` |
| `skills/acceptance/walk-product/shell/guide.html` | Verdict badges, dual `file://` / `http://` mode |
| `skills/acceptance/walk-product/SKILL.md` | Authoring flow for v2; `serve` in the hand-over |
| `skills/acceptance/walk-product/references/cases-schema.md` | v2 schema reference |
| `skills/acceptance/drive-walk/SKILL.md` | Iron Law line, two rationalization rows, CLI recipe, ask-to-stop |
| `tests/test_walk_product_cli.py` | Rewritten against v2 |
| `tests/drive-walk/scenarios-cli.md` | CLI recipes updated to v2 paths |
| `docs/agents/project.md` | Audit Trace-ignore paths; `Browser E2E` row |
| `docs/guide/skills/walk-product.md`, `docs/guide/skills/drive-walk.md` | Human docs |
...
| Path | Reason |
|---|---|
| `tests/drive-walk/fixtures/notes-app/notes-walk-product.cases.yaml` | Superseded by the `.json` fixture |
| `tests/drive-walk/fixtures/notes-app/walk-product-guide.html` | Regenerated from the v2 fixture |
...
**Files:**
- Modify: `skills/acceptance/walk-product/scripts/walk-product` — delete `load_cases_yaml` (:53-94), `load_cases_html` (:197-272), `extract_label_block` (:275-291), `extract_title` (:294-300), `extract_req` (:303-308), `strip_tags` (:311-315), `collapse_ws` (:318-320), and the `try: import yaml` block (:18-21); rewrite `load_catalog` (:37-50) and `catalog_from_dict` (:117-155)
- Create: `tests/drive-walk/fixtures/notes-app/notes-walk-product.json`
- Delete: `tests/drive-walk/fixtures/notes-app/notes-walk-product.cases.yaml`
...
RUN = REPO / "tests" / "drive-walk" / "fixtures" / "notes-app" / "notes-walk-product.json"
...
class SchemaV2Tests(unittest.TestCase):
    ...
    def test_v1_paths_are_named_errors_not_parse_attempts(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for name in ("old.cases.yaml", "old.yml", "notes-walk-product-run.md"):
                stale = Path(tmp) / name
...
Translate all six cases from `notes-walk-product.cases.yaml` to `notes-walk-product.json`
...
def load_run_file(path: Path) -> Dict[str, Any]:
    ...
    if path.suffix.lower() in {".yaml", ".yml"} or path.name.endswith("-run.md"):
        raise SystemExit(
            ...
            "run file (.skills/<slug>-walk-product.json). Delete the v1 files and re-author."
        )
    if not path.is_file():
...
`git commit -m "feat(walk-product): v2 JSON run file, drop YAML and HTML scrapers"`
...
**Files:**
- Modify: `skills/acceptance/walk-product/scripts/walk-product` — delete the ledger block `default_run_path` (:345-348), `init_ledger` (:351-375), `parse_ledger` (:378-413), `write_ledger` (:416-437) and the `HEADER_RE`/`FIELD_RE` constants (:341-342); add the store
...
`git commit -m "feat(walk-product): locked, atomic, rev-checked store"`
...
**Files:**
- Modify: `skills/acceptance/walk-product/scripts/walk-product` — `cmd_init` (:543-551), `cmd_status` (:554-568), `cmd_next` (:571-577), `cmd_mark` (:580-618), `cmd_report` (:635-668), `build_parser` (:671-731)
- Test: `tests/test_walk_product_cli.py`
...
`git commit -m "feat(walk-product): verdict commands read and write the v2 run file"`
...
**Files:**
- Modify: `skills/acceptance/walk-product/scripts/walk-product` — `render_html` (:484-513), `cmd_render` (:621-632)
- Modify: `skills/acceptance/walk-product/shell/guide.html` — CSS (:8-137), `renderCase` (:230-257), the IIFE head (:172-208)
- Create: `tests/test_walk_product_guide_contract.py`
- Test: `tests/test_walk_product_cli.py`
- Delete: `tests/drive-walk/fixtures/notes-app/walk-product-guide.html` (regenerated)
...
`python3 skills/acceptance/walk-product/scripts/walk-product render tests/drive-walk/fixtures/notes-app/notes-walk-product.json -o tests/drive-walk/fixtures/notes-app/walk-product-guide.html`

- [ ] **Step 3: Commit**

`git commit -m "feat(walk-product): render bakes verdicts; guide declares its snapshot"`
...
### Task 5: `walk-product serve` — the loopback layer

**Files:**
- Modify: `skills/acceptance/walk-product/scripts/walk-product` — add `cmd_serve` and the handler; register the subparser in `build_parser`
...
`git commit -m "feat(walk-product): loopback serve layer with human-scoped writes"`
...
**Files:**
- Modify: `skills/acceptance/walk-product/scripts/walk-product` — `cmd_serve` gains pidfile write and `--stop`
...
`.skills/<slug>-walk-product-serve.pid` with `secrets.token_hex(16)`. `verify_live` GETs
...
`git commit -m "feat(walk-product): token-verified server shutdown"`
...
**Files:**
- Modify: `skills/acceptance/walk-product/shell/guide.html` — the IIFE gains the polling branch
...
`git commit -m "feat(walk-product): live polling and tick POST in the served guide"`
...
**Files:**
- Modify: `skills/acceptance/drive-walk/SKILL.md` — Iron Law second line (:26), CLI recipe (:47-67), failure routing reference to cases YAML (:137), §5 close (:155-163), rationalization rows (:170-171), red flags (:180-185)
- Modify: `skills/acceptance/walk-product/SKILL.md` — §4 authoring (:82-113), §5 hand-over (:115-125), red flags (:141-145)
- Modify: `skills/acceptance/walk-product/references/cases-schema.md` — full rewrite for v2
- Modify: `tests/drive-walk/scenarios-cli.md`, `docs/agents/project.md`, `docs/guide/skills/walk-product.md`, `docs/guide/skills/drive-walk.md`, `CHANGELOG.md`
...
def test_drive_walk_asks_before_stopping_the_server(self):
    ...
    text = (REPO / "skills" / "acceptance" / "drive-walk" / "SKILL.md").read_text()
    self.assertRegex(text, r"route-work .*whether to stop|serve --stop")
...
Run: `python3 -m unittest tests.test_walk_product_cli` — expect: the route-work assertion fails and
...
`tests/test_walk_product_guide_contract.py`. Keep `tests/drive-walk/fixtures/` and
`tests/drive-walk/scenarios-cli.md` ignored — those still carry fixture IDs as
data. Then prove it:
...
`git commit -m "docs(walk-product): skill bodies and references for the v2 run file"`

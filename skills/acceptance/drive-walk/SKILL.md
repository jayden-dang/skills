...
Execute an existing walk-product guide against the **product app** in a real browser.
...
"Walk Product judges product behavior on screen, not wire traffic" is false for any
...
progress — that burns tokens and writes to the wrong field space. Use `walk-product
mark`.
...
Resolve the walk-product skill root (this monorepo:
`skills/acceptance/walk-product`; when installed, the skill package path). Every
subcommand takes the **one** run file — cases and verdicts live in it together:

```bash
DF="python3 <skill-root>/scripts/walk-product"
RUN=.skills/<slug>-walk-product.json
...
- Do **not** open the walk-product guide HTML as a drive target.
...
The run file is the one `walk-product` wrote: `.skills/<slug>-walk-product.json`.
...
2. `$DF report $RUN -o .skills/<slug>-walk-product-report.md`
...
| Thought | Reality |
|---|---|
| "Walk Product judges the screen, not wire traffic" | State cases require a server probe. Screen-only is not a pass. |
...
| "User said whatever is fastest / demo in N minutes" | Speed is not consent for staging/prod. Route Work, or run local. |
...
- Opening the walk-product HTML in a browser to tick checkboxes during the run
...
- Patching product on a walk-product fail without `root-cause` when the fail is deterministic

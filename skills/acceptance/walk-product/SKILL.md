...
description: Use when manually exercising a finished feature in the real running app from
  ...
  checkable, resumable walk-product guide (one JSON run file + rendered HTML) kept
...
1. Write `.skills/<slug>-walk-product.json` (schema: load sibling
...
2. Render the human guide from the checked-in shell — **do not** load
   ...
   ```bash
   python3 <skill-root>/scripts/walk-product render .skills/<slug>-walk-product.json \
     -o .skills/<slug>-walk-product.html
   ```

   Resolve `<skill-root>` to this skill's install path (in this monorepo:
   `skills/acceptance/walk-product`). The shell is `shell/guide.html` — theme-aware
...
```bash
python3 <skill-root>/scripts/walk-product serve .skills/<slug>-walk-product.json
```
...
If the **agent** should run the guide, name `drive-walk` and the run-file
path.
...
| "One happy case per requirement is enough" | The coverage gate requires non-happy kinds (or a written exception). Happy-only is a demo, not walk-product. |
| "Edges belong in unit tests, not the guide" | Walk Product is the user-facing surface. If the user can hit the edge, it gets a row. |
| "Worse cases mean load/chaos/fuzz" | Those are other harnesses. Walk Product worse cases are edge, error, nonbehavior, persist. |

## Red Flags

- Hand-writing a full HTML/CSS page instead of the run file + `walk-product render`
- Missing `backend` / `setup` / `kind` on any case
- Happy-only section without a greppable coverage exception
- Telling the agent to mark progress via guide ticks instead of `walk-product mark`

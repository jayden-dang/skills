...
Load when authoring or validating a walk-product run file. One JSON file per run holds
the authored cases **and** the verdicts, so what the agent records is what the
person reads.

## Path

`.skills/<slug>-walk-product.json`
...
|---|---|---|
| `run` | `verdict` (`pending`/`pass`/`fail`/`blocked`), `saw`, `server`, `notes` | the agent, via `walk-product mark` |
...
never advances `walk-product next`, and no flag makes it evidence — see
...
```bash
DF="python3 skills/acceptance/walk-product/scripts/walk-product"
...
$DF render $RUN -o .skills/<slug>-walk-product.html

...
4. **Agent dogfood** — name `run-flow-guide` and the run-file path
   **only after** a clean `vet-flow-guide` report (or a named override on open
...
case is fully slotted, and `.skills/<CODE>/vet-flow-guide.md` exists for this
run file.*

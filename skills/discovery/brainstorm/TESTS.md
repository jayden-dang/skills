# `brainstorm` — knowns inventory + blindspot (unknowns loop)
...
| S-BS-U3 taste / feel | Multi-variant prototype before lock |
...
- `grilling` — blast-radius first already held under "ask color first" authority;
  explicit sentence added for connectivity, not a new RED failure.
- `prototype` / `research` — remain detours for unknown knowns / known unknowns.
- `interpret` — map/territory technique pass (with skill): challenged OAuthProvider

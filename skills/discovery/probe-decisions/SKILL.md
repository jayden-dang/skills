...
1. **Radius** — one of: `architecture` · `data` · `auth/security` · `UX flow` · `polish-diff` (label it).
...
- **Right-size to posture.** When the parent supplies it, or `docs/agents/project.md` has **Project posture**: skip data migration / backward compatibility / deprecation on Run Spike · Research · Learning; press them on Cut Released · Scaling · Maintenance. Absent posture, walk every branch. Posture and Team **band** are orthogonal.
...
Probe Decisions owns the **interview** leg of pre-implementation unknowns work. Other legs are open-set *sources* or handoffs — not extra fixed rounds:

| Leg | Probe Decisions does | Does not re-own |
|---|---|---|
...
| "Context can be a follow-up if they route-work" | The card *is* the detail. Follow-up-only context is a thin-card failure. |
...
| "Senior said switch cleanly into probe-decisions and park the parent" | Nesting *is* the clean switch. Parking the parent and opening a probe-decisions checklist is dual-channel thrash. |
| "A short decision checklist under probe-decisions isn't a competing list" | It is a second list. Decision areas live as the parent's in-progress interview item. |
| "Announce Using probe-decisions so the user sees the write-handoff" | Nested: no mode-switch announcement. Standalone (no parent): you may name probe-decisions once. |

## Red flags — stop and rewrite the turn

- Calling `AskUserQuestion` or any truncated MCQ tool for a probe-decisions decision
...
- Route Working the user for a fact present in the repo or the parent's scan digest

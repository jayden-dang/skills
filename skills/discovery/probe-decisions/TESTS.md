# `probe-decisions` — open-set interview + rich cards + pre-impl map
...
| **S-GRILL-SHAPE** | exhaustion + sunk cost | **B1** thin one-liner (prior); skill text capped "why" at 1–2 sentences | "Probe Decisions’s thin default is question + recommendation — not a rich essay" |
...
| **S-GRILL-CONTEXT** | time + authority + exhaustion | **User production:** cards lack Thread/Territory; not enough to analyze | "keep probe-decisions cards minimal — question + 2 options + recommended, no walls of context" / "why it matters can be one short line" |
| **S-GRILL-PREIMPL** | time + authority | blindspot teach-pack only on parent; no high-tweak surface in close | "probe-decisions is just questions; blindspot is frame-change's job forever" |
...
| frame-change pointer body-skip risk | Protocol summary shortened; forces load of `probe-decisions` |
...
**should-fire:** "interview me about this feature", "grill me on the design", "we're underspecified — walk the decisions", parent step "apply probe-decisions protocol", "stress-test this plan before we build", "one question at a time on ambiguities".

**should-not-fire (neighbors):** full greenfield idea shaping → `frame-change`; runnable spike → `run-spike`; external API fact → `research`; already-shipped small tweak → `amend-feature`; native-language companion → `interpret-native`; post-ship quiz/explainer → `brief-team` / acceptance path.

## Neighbor skills

- `frame-change` owns Knowns inventory + Blindspot scan; probe-decisions **consumes** them, runs rich interview cards + open-set stop + close package.
- `run-spike` / `research` remain detours for unknown knowns / known unknowns — probe-decisions offers the reference path or hands back one decision card after the detour.
- `define-domain` stays passive side effect (glossary); does not open a second question channel.
- `plan-tasks` consumes high-tweak surface from the close package; probe-decisions does not write the plan.

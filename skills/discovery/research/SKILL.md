...
2. Adversarially verify before reporting. For each key claim, run one search phrased to *negate* it, and one over the history of the thing under research — the library's changelog or issue tracker, or for a standard or platform, its revision history or errata — for the version where the claim stopped holding. State both queries and what each returned, even when that is nothing. A claim whose negating search was never run is reported as unverified, not as fact. Note conflicts explicitly rather than picking a side silently.

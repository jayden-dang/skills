...
Before any code: one paragraph at the top of the run-spike (comment or adjacent README) stating the model under test and the question it must answer. A logic run-spike aimed at the wrong question is pure waste, and the written question is what the verdict gets checked against later.
...
**Direction of dependency is absolute: the TUI imports the logic module; nothing flows the other way.** The logic module is the only part of the run-spike that may outlive it — once validated, it can be lifted into real code while the TUI shell is deleted.
...
Wire it into the project's existing task runner so the user launches it by name, never by path. No runner? Put the exact command on the first line of the run-spike's README.

### 6. Hand it over, then listen

Give the user the command and let them drive. The valuable moments are "wait, that shouldn't be allowed" and "I expected X here" — those are defects in the *idea*, which is exactly what you're hunting. Add actions on request; run-spikes evolve.
...
- **Tests on a run-spike.** If it needs tests, it has stopped being a run-spike.
- **Wiring to a real database.** In-memory only, unless persistence *is* the question.
- **Generalizing.** No "we might need X later." One question, one run-spike.

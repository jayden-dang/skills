...
- **Last resort: a new throwaway route**, only when there is genuinely no page to embed in (an entirely new top-level surface). Follow the project's routing conventions, put `run-spike` in the path or filename, and sanity-check first: is there really nowhere to embed this?

## Process

### 1. State the plan and pick N

Default **3 variants; never more than 5** — beyond that they stop being different and start being noise. Record the plan in one line at the run-spike's location, e.g. "Three variants of the billing screen on the existing `/billing` route, switched by `?variant=`."

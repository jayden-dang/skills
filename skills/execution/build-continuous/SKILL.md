...
| Intent | Use instead |
|---|---|
| `Execution-mode: story-unit` (human-gated review units) | REQUIRED SUB-SKILL: use `build-story-units` |
...
| `story-unit` | Do **not** run this skill's loop. REQUIRED SUB-SKILL: use `build-story-units` |
| missing / `unset` / invalid | Ask the user; write `continuous` or `story-unit` into `tasks.md` (commit if tracked). If they pick `story-unit` → `build-story-units`. If `continuous` → proceed. Never invent continuous silently. No project-level default |

Never run unit barriers, unit derivation, or human unit stops in this skill.
Those live only in `build-story-units`.

| Thought | Reality |
|---|---|
| "build-continuous used to handle both modes — I'll keep going in story-unit" | Story-unit is `build-story-units`. Staying here is the dual-mode failure |
| "I'll derive units as a size signal then continuous" | No unit table in this skill. Waves come from Depends-on only |
| "Lead wants human stops — stay in continuous and pause anyway" | Pause policy is the header. Write `story-unit` and use `build-story-units`, or keep continuous with no pauses |
| "PM / standup — default continuous without asking" | Empty mode means not ready; route-work and write the field |

## Setup

1. **Mode gate first.** Parse `Execution-mode:` per the table above. *Done when:
   you are on continuous, or you have handed off to `build-story-units`.*
2. **Workspace check — route-work first.** If no isolated workspace exists yet, ask:
...
4. **Acceptance.** REQUIRED SUB-SKILL: use `validate-feature`. Breaks → `root-cause`,
   then promote to committed ID-tagged tests. Optional: `/sample-attention`
   (not a gate).

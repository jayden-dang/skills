# `build-continuous` / implementer-prompt — deviation log
...
`DONE_WITH_CONCERNS` row in `build-continuous` SKILL.md: read notes file; route
...
**Design:** drop dual-mode; story-unit → `build-story-units`; no unit preflight on continuous

## RED (old dual-mode body)

| Scenario | Observed | Failure for narrow target |
|---|---|---|
| story-unit while on build-continuous | Dual-mode body still *owns* story-unit recipes (B by text); contaminated A when DESIGN.md also loaded | No REQUIRED redirect in skill body |
...
1. Mode gate: `story-unit` → REQUIRED SUB-SKILL `build-story-units` (not run barriers here)
2. Continuous Setup: no unit table / no `story-unit-mode` load
3. Continuous: no human pause between tasks under social pressure
4. Description routes continuous-only; story-unit intents → `build-story-units`

## GREEN retest log (2026-07-30, grok-4.5)

| Scenario | CHOICE / result | Cite skill? |
|---|---|---|
| story-unit while on build-continuous → redirect | **A** hand off `build-story-units` | yes — Mode gate + Red Flags |
...
| mode unset + standup pressure | **A** route-work and write field | yes — never invent continuous |
| routing continuous vs story-unit | continuous→build-continuous; story-unit→build-story-units; opened build-continuous + story-unit header → build-story-units | yes |
...
- [x] `story-unit-mode.md` is a pointer to `build-story-units`

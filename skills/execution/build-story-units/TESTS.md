# `build-story-units` — test evidence
...
|---|---|---|
| All plan-execution intents route only to `build-continuous` | Distinct trigger (split case 1) | description + opening "not continuous / not inline" |
...
**Vs RED bare:** mode-change write-back was Partial/missing under dual-mode freeform; both GREEN reps include `write Execution-mode: continuous into tasks.md` + write-handoff to build-continuous.
...
| S-STORY-2 | grok-4.5 | A | yes — mode change + build-continuous write-handoff |

### Description routing

| Query | Expected | Result |
|---|---|---|
| continuous + subagents | build-continuous | build-continuous |
| story-unit + review after each story | build-story-units | build-story-units |
| no subagents | build-inline (future) | build-inline named (not shipped) |
| suite failing mid-run | root-cause | root-cause |
| continuous just run it | build-continuous | build-continuous |
| story-unit unit barriers | build-story-units | build-story-units |
...
- [x] No-op + duplication: per-task loop inlined intentionally until build-continuous narrow extracts shared home; templates pointer to `../build-continuous/*-prompt.md`
- [ ] build-continuous still dual-mode until its own RED/GREEN narrow pass

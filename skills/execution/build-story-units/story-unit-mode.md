...
9. **Hard fail preflight** — task with zero parseable IDs; task citing only struck
   IDs. **Empty non-NFR story** (no tasks) → omit unit + report (aligns with audit-trace
...
3. Dispatch **task reviewer** once at **unit scope** (two-verdict) over that
   range, using `../build-continuous/task-reviewer-prompt.md`. Human is never first
...
- **stop stopping** / **just run it all** → I write Execution-mode: continuous
  into tasks.md first, then hand remaining work to build-continuous (chat-only is
  not a mode change)
```

...
2. **Build the brief.** Copy Task N's block plus the verbatim Global Constraints into `.skills/task-N-brief.md`. That file is the implementer's entire world — it must carry every exact value (numbers, magic strings, signatures) the task needs. When a `docs/architecture/` spine exists, also list in the brief the `**ARCH-N**` invariants relevant to this task's touched files, so the implementer builds within them; no spine, add nothing. When Team **band** was loaded in Setup: apply **packaging** to brief tone (Solo = pair-with-user; Small/Multi = owners when notes apply) — never omit review obligations. *Done when: the brief file exists.*

...
2. **Build the brief.** Run `task-brief <tasks.md> N` (repo script; it extracts Task N plus the Global Constraints into `.skills/task-N-brief.md` and prints the path). No `task-brief` on PATH or in `scripts/`? Build the brief by hand: copy Task N's block plus the verbatim Global Constraints into `.skills/task-N-brief.md`. *Done when: the brief file exists.*
...
6. **Package the diff.** Run `review-package $BASE $(git rev-parse HEAD)` (repo script; prints the path of the bundle it wrote). No `review-package` available? Assemble the bundle by hand into `.skills/review-<base7>..<head7>.diff`: `git log $BASE..HEAD --oneline`, `git diff --stat $BASE HEAD`, then `git diff $BASE HEAD`. BASE is the sha from step 1 — **never `HEAD~1`**, which silently drops every commit of a multi-commit task except the last. *Done when: the package path is printed.*
7. **Dispatch a task reviewer** using `task-reviewer-prompt.md` (beside this file) with: the brief path, the report path, the diff package path, the Global Constraints verbatim, and an explicit model. One reviewer produces both verdicts here: a single-task diff is small enough that keeping the two axes explicitly separate prevents cross-masking without paying for two subagents — the whole-branch `code-review` later uses two parallel agents, where a large diff is where that isolation earns its cost. *Done when: both verdicts are back.*

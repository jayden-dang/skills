...
2. **Build the brief.** Copy Task N's block plus the verbatim Global Constraints into `.skills/task-N-brief.md`. That file is the implementer's entire world — it must carry every exact value (numbers, magic strings, signatures) the task needs. WHERE a `docs/architecture/` spine exists, also list in the brief the `**ARCH-N**` invariants relevant to this task's touched files, so the implementer builds within them; no spine, add nothing. *Done when: the brief file exists.*
...
7. **Dispatch a task reviewer** using `task-reviewer-prompt.md` (beside this file) with: the brief path, the report path, the diff package path, the Global Constraints verbatim, and an explicit model. WHERE a `docs/architecture/` spine exists, also run `check-invariants` (REQUIRED SUB-SKILL) on the task diff; a `violates` verdict is a finding routed through the fix loop (step 8) like any other. No spine, skip it. *Done when: both verdicts are back.*

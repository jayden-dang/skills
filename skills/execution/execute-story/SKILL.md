...
without unit barriers is `execute-plan`. Controller-implements-without-subagents
is `execute-inline`. If `Execution-mode:` is `continuous`, stop and use
`execute-plan` instead. If the user wants no subagents, name `execute-inline`.
...
3. **"continue"** after looking = next unit only. **"stop stopping" /
   ...
   first, then hand remaining work to `execute-plan`. Chat-only is not a mode
   change.
...
1. **Mode gate.** Read `Execution-mode:` from `tasks.md`. Must be `story-unit`.
   If `continuous` → use `execute-plan`. If missing/`unset`/invalid → ask, write
   the chosen value, proceed only if the result is `story-unit`. Never invent
   continuous. *Done when: header is `story-unit`.*
2. **Workspace check — ask first.** Isolate in a worktree, or implement on the
   current branch? Do not create a worktree unasked. Isolation → REQUIRED
   SUB-SKILL: use `worktrees`. Current branch is main/master → separate
...
3. **Dispatch a FRESH implementer** using the template at
   `../execute-plan/implementer-prompt.md` (one home for the implementer
...
7. **Task reviewer** via `../execute-plan/task-reviewer-prompt.md` with brief,
...
8. **Fix loop.** Critical/Important → fix subagent (re-run covering tests under
   `tdd`, append to same report) → **re-review**. Same finding survives 3
   cycles → stop; if plan/design/requirements invalidated, REQUIRED SUB-SKILL:
   use `correct-course`; else escalate. Never fix in controller context. Minors
...
| stop stopping / just run it all | Write `Execution-mode: continuous` into `tasks.md` (commit if tracked). Ledger this unit complete. REQUIRED SUB-SKILL: use `execute-plan` for remaining tasks (no further unit barriers). |

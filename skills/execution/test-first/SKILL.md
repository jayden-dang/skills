...
No requirement covers the behavior? For tier ≥1 work that is a gap — surface it to the user rather than shipping an untraceable test. If `docs/agents/project.md` is missing, say so, suggest running `configure-repo`, and fall back to putting the ID in the test name.

...
6. REQUIRED: fix `artifact_shape` for the whole cycle — genre, length band, time limit, medium. `lang-write-artifact` holds this constant so weeks stay comparable; decided here, once.
...
8. Write `cycles/C<NN>.md` with `status: active`. Close the previous cycle only through `lang-assess-level`.
...
A focus id that does not exist in `capability-map.md` costs a whole cycle silently: `lang-run-session`
pulls nothing for it, and `lang-review-practice-week` reports zero attempts on a row that was never

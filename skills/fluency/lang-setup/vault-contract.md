...
Load this file at step 2 of `lang-setup`, and run its check at the last step.
...
`profile.md`'s two metric tables are not optional decoration: `lang-diagnose-output` appends the
calibration row and `lang-run-session` appends the translation ratio every session. Without them

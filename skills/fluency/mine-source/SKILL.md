...
through `build-lexicon`, one shadowing segment with its phonology rows, gaps each labelled
...
3. Extract at most `limits.chunks_per_source` **chunks**: collocations, functional phrases, whole clauses that carry a move. REQUIRED SUB-SKILL: use `build-lexicon` to file them — it holds the rule for what counts as a chunk.
...
Source note written with comprehension share, chunks filed through `build-lexicon`, a shadowing segment with its phonology rows, mechanism-labelled gaps, and one production task reusing ≥3 chunks.

...
7. **The learner writes their own sentences** — one per context type, about their own work or life. Do not draft these; diagnose them. REQUIRED SUB-SKILL: use `diagnose-output` if any sentence needs correcting.
8. Write `lexicon/<slug>.md`. REQUIRED SUB-SKILL: use `build-lexicon` to file the ledger row, link the study note from it, and set the first `next_due` bucket.

## Review, later

The study note is the depth; the ledger row is what brings it back. `run-session` pulls due
rows into production tasks and `run-voice-session` steers them into conversation. Neither

...
4. Default routing when the learner just says "let's practise": spoken turn → `run-voice-session`; typed turn → `run-session`.
...
6. Gap check: `last_session` older than `config.schedule.recovery_gap_days` → route to `run-session`, which opens in recovery mode.

## Routing

| Learner says | Skill |
|---|---|
| "practise", "let's study", typed drill | `run-session` |
...
| end of month or cycle | `assess-level` |
| opening a new cycle | `plan-cycle` |
| no vault yet | name `setup-fluency-os` for the learner to run |
...
| "They missed a week — open everything overdue" | Route to `run-session`; recovery mode is defined there |

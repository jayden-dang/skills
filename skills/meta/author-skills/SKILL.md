...
A skill wrangles determinism out of a stochastic system. The root virtue is **predictability**: the agent takes the same *process* on every run — not that it emits identical output. A frame-changeing skill should reliably diverge; its tokens vary, its behavior doesn't. Every rule in this document is a lever on predictability.
...
- **Verb-first names**: `author-skills`, `vet-feedback`, `root-cause` — name the action, not the topic. Engineering skills carry no prefix: they are the default vocabulary. The life pack is namespaced instead — `life-` (Personal OS) — because its bare verbs (`capture`, `orient`, `replan`) collide with engineering names once both packs are installed. **Verb-first applies after the prefix**: `life-plan-day`, `life-review-week`. A prefix is a disambiguator for the human picking from a `/` menu, not a routing device — discovery still runs on the description, per the keyword rule below.
...
- User-invoked skills carry `disable-model-invocation: true` — so they **cannot be auto-invoked**, and no skill body may tell the agent to *invoke* one. A hand-off reaches a user-invoked skill only by naming it for the user to run (`/write-handoff`, `/cut-release`); `REQUIRED SUB-SKILL: use \`x\`` is for model-invocable targets only. Directing the agent to invoke a `disable-model-invocation` skill is a dead-end hand-off — a real bug, not a style nit.
...
- [ ] Core body within token budget (≤300 lines, `lint-skill-length.py` green; past 200, say why the body needs the room); reference disclosed behind well-worded pointers, one level deep; any reference >100 lines has a TOC

...
- **Verb-first names**: `author-skills`, `vet-feedback`, `root-cause` — name the action, not the topic. Engineering skills carry no prefix: they are the default vocabulary. The two life packs are namespaced instead — `life-` (Personal OS) and `lang-` (Fluency OS) — because their bare verbs (`capture`, `orient`, `replan`, `run-session`) collide with engineering names and with each other once both packs are installed. **Verb-first applies after the prefix**: `life-plan-day`, `lang-study-word`. A prefix is a disambiguator for the human picking from a `/` menu, not a routing device — discovery still runs on the description, per the keyword rule below.

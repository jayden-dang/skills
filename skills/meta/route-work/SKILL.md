...
`validate-feature`, `walk-product`), invoke it and let it take over. If it is a user-invoked skill
(`triage`, `publish-issues`, `scan-architecture`, `configure-repo`,
`bootstrap-repo`, `anchor-project`, `status-roadmap`, `assess-milestone`, `write-handoff`, `cut-release`), you cannot invoke it — name it and
...
3. **`isolate-workspace`** → **`build-continuous`** — isolated workspace, then task-by-task
...
- A mid-execution discovery invalidated your already-approved plan (the plan is wrong, scope
  ...
  lowest invalidated artifact and routes the re-entry; `build-continuous` also hands off to it).
...
- Want to try a finished feature by hand in the real app → **`walk-product`**
...
- Starting a large/long-lived project and want a product vision + architecture-invariant
  spine above the feature loop → **`anchor-project`** (optional; the layer the feature
...
- Several milestones in flight and unsure where the plan stands or what to do next →
  **`status-roadmap`** (read-only; derives progress from the roadmap, the specs, and git, then

...
**Priority:** process skills first (`brainstorm`, `debug`, `triage`), then
implementation skills. "Build X" → `brainstorm` before anything. "This is
broken" → `debug` before any fix. Unsure which flow fits → `ask`.

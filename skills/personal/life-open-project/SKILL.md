...
7. Offer `life-plan-project` for milestones (do not invent a 3-month task tree).

If they only have a name: REQUIRED SUB-SKILL path is user runs `life-capture` — park in inbox, not active.
...
| "Just register quickly" | Active slots are scarce — use `life-capture` if unready |

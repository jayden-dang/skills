...
not need it — the feature workflow (`frame-change` → spec → `build-continuous`) works fully
without it, and nothing here is a gate. If the user is not sure they need it, say so and
let them decide.

## Modes

Pick the mode from what the user asked; route-work if it is unclear.
...
2. **Interview.** REQUIRED SUB-SKILL: use `probe-decisions` — one question at a time — to draw
   ...
   after the user ratifies it in the `probe-decisions` channel; unratified candidates are
...
- **Pivot with shipped collisions.** WHERE the new direction contradicts a
  ...
  non-goal/hard constraint, **stop** and name `/dispose-pivot` for the user to
...
- REQUIRED SUB-SKILL: use `judge-invariants` across the feature `design.md` files to
...
exists, say the project has no layer and that `/anchor-project` (create mode) can add

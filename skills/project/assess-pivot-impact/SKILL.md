...
**Where this sits:** after the user has a new direction; **before** `/define-project`
update rewrites `docs/product/vision.md` or `docs/architecture/`. Opposite of
`realign-spec` (code is truth → fix the spec) and of brownfield `define-project` create
...
wrong tool. Name `/define-project` (update) for the user and stop.
...
`define-project` remains the sole writer of the vision layer. `plan-milestones` remains
the sole writer of `docs/roadmap/INDEX.md`. After the ledger is confirmed, **name**
`/define-project` (update) for the user to run — agents never auto-run it
(`disable-model-invocation: true`).
...
1. Name **`/define-project`** (update mode) for the user — that skill owns
...
*Done when: the user has a confirmed ledger and a clear next command (`/define-project`),
...
| "I'll write the vision myself; define-project is just ceremony" | That file has a single writer. Name `/define-project` |
...
- You are about to open `/define-project` yourself instead of naming it for the user
...
- No project-docs layer **and** no `docs/specs/INDEX.md` → nothing to repoint; name
  `/define-project` (create) or the feature workflow, and stop.
- New intent collides with nothing shipped → name `/define-project` (update); this
...
`/define-project` (update) for the doc rewrite.

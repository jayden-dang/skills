---
name: repoint-project
description: Produces a disposition ledger when a product pivot puts shipped code at
  odds with a new vision or architecture. Run it with /repoint-project.
...
skill owns the handoff from "we decided to pivot" to "every live commitment has a
fate." It does not rewrite the vision layer. It produces the ledger that makes a
rewrite honest.

**Where this sits:** after the user has a new direction; **before** `/establish-project`
update rewrites `docs/product/vision.md` or `docs/architecture/`. Opposite of
`sync-spec` (code is truth → fix the spec) and of brownfield `establish-project` create
(code is truth → ratify it). Opposite of `improve-architecture` (vision-neutral
deepening). Mid-plan invalidation stays with `correct-course`.
...
wrong tool. Name `/establish-project` (update) for the user and stop.
...
| **Accept debt** | known contradiction; keep living with it (ADR required later via `domain-modeling`) |
| **Unknown** | not enough evidence → stop that row; name `research` or `prototype` |
...
1. Name **`/establish-project`** (update mode) for the user — that skill owns
   `vision.md` / `architecture/` writes, goal/ARCH ID immutability (strikethrough, never
   renumber), and ADRs via `domain-modeling`.
...
3. If a roadmap exists and goals moved, name that `write-roadmap` will need a pass after
   the vision layer updates — do not edit `docs/roadmap/INDEX.md` here.

*Done when: the user has a confirmed ledger and a clear next command (`/establish-project`),
...
| "I'll write the vision myself; establish-project is just ceremony" | That file has a single writer. Name `/establish-project` |
...
- You are about to treat code as truth and rewrite the spec to match → that is `sync-spec`
...
- No project-docs layer **and** no `docs/specs/INDEX.md` → nothing to repoint; name
  `/establish-project` (create) or the feature workflow, and stop.
- New intent collides with nothing shipped → name `/establish-project` (update); this
  skill stops without a ledger.
- Mid-execution plan invalidation only → `correct-course`, not this skill.
...
`/establish-project` (update) for the doc rewrite.

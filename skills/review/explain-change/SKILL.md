...
VERIFY THE WRITTEN FILE BEFORE ANY SUCCESS CLAIM
...
When verify fails, never withhold merge, PR, discard, or review options — report
...
| "Soft-gate finish-branch until they run it" | Callers only **name** this skill; the naming predicate is theirs, not yours. Never auto-run. Never block the menu. |
...
11. **Verify the written file** — a fixed grep pass over the path you just wrote, before any success claim:
    ...
    for k in users decisions breaks verify intuition seams; do
...
12. **Handoff** — report the path and that INDEX was updated. Never claim quiz pass/fail. Never claim merge is blocked pending this file.
...
Callers **name** `/explain-change` for the user; the predicate that decides when

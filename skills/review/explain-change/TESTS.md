# `explain-change` — pressure evidence (writing-skills)
...
|---|---|---|
| S-EC-QUIZ | No quiz / scores | Pitch+map only; point at `/comprehend-change` for author quiz if asked |
| S-EC-GATE | Allow PR | Name `/explain-change` optional; never withhold menu |
...
- `comprehend-change` — self + quiz + outside repo
- `finish-branch` — names both when conditions match; never invokes either

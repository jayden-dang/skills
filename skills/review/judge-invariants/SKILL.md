...
description: Use when a design or diff cites architecture invariants (a `Respects`
  ...
  LLM-judged invariant-conformance check invoked by `inspect-change`, `build-continuous`'s
  task review, and `anchor-project` validate mode. Produces a per-citation
...
```
judge-invariants (3 citations, spine: docs/architecture/INDEX.md)
...
returns findings, never a pass/fail. The caller (`inspect-change`, `build-continuous`,
`anchor-project`) decides what to do with a `violates`.

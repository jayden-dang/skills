...
2. **Correctness bugs are out of scope.** Do not hunt them, and do not fix the ones you trip over. Record them and name `inspect-change` (or `root-cause`, if something is already broken) for the user to run. A diff that both restructures and repairs can be reviewed as neither.
</NON-NEGOTIABLE>
...
Fix each surviving finding directly, smallest blast radius first. Stay inside the pinned diff's files. REQUIRED SUB-SKILL: use `test-first` — its refactor discipline is exactly this step: the tests stay fixed while the code underneath them moves. *Done when: every queued finding is applied or has moved to the dropped list with a reason.*

## 6. Prove behavior held

Re-run step 2's exact command. It must be green, and the only test-file change permitted in `git diff` for this cleanup is a **deletion** meeting all three conditions below — **any other test-file line, added or removed, means stop**. REQUIRED SUB-SKILL: use `prove-claim` — "the cleanup looks safe" is not evidence, and a refactor is the single easiest place to break something while feeling certain you did not. A red suite means revert the offending fix, not repair it forward.

The permitted deletion, and its three conditions: a test may be **deleted** when the only thing it pinned was code the Simplification angle removed as dead — the target is gone, the behavior is still covered elsewhere by a test you **name**, and the deletion is stated in step 7's report. Fewer than three met, the test stays. A test may never be *rewritten, retargeted, renamed, or moved*; that is the goalpost move rule 1 forbids, and "I retargeted the same assertion at the canonical function" is that move. If a test resists deletion, keep the old symbol as a thin alias delegating to the canonical one — the duplicated *rule* dies, which was the finding, and the suite never moves.
...
| Thought | Reality |
|---|---|
| "That's obviously a bug — it'd be silly to leave it" | Then it deserves a real review, not a drive-by fix buried in a cleanup commit. Name it for `inspect-change`; a diff that restructures *and* repairs can be reviewed as neither |
| "The test asserted the old internals, so I updated it" | You changed behavior and moved the goalposts that would have caught it. Revert. If a test genuinely must change, this is not polish-diff |
...
| "Behavior technically changed, but strictly for the better" | That is a design change wearing a cleanup's clothes. Route it to `amend-feature` and let it be seen |

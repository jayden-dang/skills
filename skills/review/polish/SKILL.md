...
Re-run step 2's exact command. It must be green, and the only test-file change permitted in `git diff` for this cleanup is a **deletion** meeting all three conditions below — **any other test-file line, added or removed, means stop**. REQUIRED SUB-SKILL: use `verify` — "the cleanup looks safe" is not evidence, and a refactor is the single easiest place to break something while feeling certain you did not. A red suite means revert the offending fix, not repair it forward.

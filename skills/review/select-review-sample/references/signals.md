...
its own path, matching `audit-trace`'s recognition rules:
...
**Do not use `audit-trace`'s root list** (`tests test e2e src src-tauri crates app lib
packages`) for this. Those are the directories `audit-trace` *searches*; `src/`,

...
- Single global substrate and single logical audit-trace target for decision records
...
carrier is the crossing's own tracked evidence — the commit, tag, PR, or cut-release
...
| `Emitter:` | e.g. `land-branch`, `cut-release` |
...
| `Evidence:` | repeatable; `<kind> <value>`; kind ∈ `commit` \| `tag` \| `pr` \| `ci` \| `cut-release`; tag value is `<ref-name>@<object-id>` |
...
| Token | Typical use |
|---|---|
| `integration` | land-branch merge |
| `publication` | outbound PR |
| `disposal` | discard |
| `cut-release` | cut-release approve/reject |

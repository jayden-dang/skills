...
1. Caller is `land-branch` or `cut-release` (closed set)
2. Write Handoff includes a **terminal human verdict**
...
| Thought | Reality |
|---|---|
| "This approval is important enough to record" | Only land-branch / cut-release with a terminal verdict |
...
- Durable citations (`commit`, `tag name@object-id`, PR/CI/cut-release refs)
...
| "I'll polish-diff their wording" | Polish Diff is fabrication under the **verbatim** law |
...
6. Withhold without locator → route-work **once**; if declined/unavailable →
...
```bash
sh skills/ship/record-verdict/validate-records.sh --scan
```
...
```bash
sh skills/ship/record-verdict/validate-records.sh --mode=publish --record <file>
```
...
- Publishing without a terminal human verdict or outside {land-branch, release}

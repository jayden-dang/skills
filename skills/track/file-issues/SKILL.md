...
- **Edge is only text** — "Blocked by" is a line in the body with nothing enforcing it (GitHub Issues, local files). Label only the **frontier** — slices with no open blocker — `ready-for-agent`; give each blocked slice its category label alone — or, where there is no label mechanism at all (local `.scratch/` files), write `blocked` in its Status field — and tell the user to promote each one as its blockers close.

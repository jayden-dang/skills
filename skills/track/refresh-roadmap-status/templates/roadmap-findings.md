...
Authoritative. Read by `refresh-roadmap-status`, which reports these findings, and by
...
Resolve this file in this order, first path that exists: (1) `templates/roadmap-findings.md`
beside the reading SKILL.md, (2) `${CLAUDE_PLUGIN_ROOT}/templates/roadmap-findings.md` when
that variable is set, (3) `../../../templates/roadmap-findings.md` relative to the reading
SKILL.md.

<!--
Not a seed. Unlike the other files in this directory, nothing copies this one into a
consumer repo — `configure-repo` and `define-project` name the template files they write
...
of the findings themselves — that rule lives in its skill body. `refresh-roadmap-status` reports the

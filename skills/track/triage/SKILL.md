...
1. **Gather context.** Read the full issue — body, comments, labels, author, dates; for a PR, the diff. Read any prior triage notes so you never re-ask an answered question. Explore the relevant code using the glossary in `CONTEXT.md`. When `docs/architecture/` exists and the issue touches a spine it covers, REQUIRED SUB-SKILL: use `inspect-invariants` for the conformance read rather than judging it here. Then run two checks:

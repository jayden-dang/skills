# RED baselines — assess-pivot-impact

Recorded failures that justify each behavior the skill adds. Fixture:
`tests/assess-pivot-impact/fixture-ledgerly` (Ledgerly personal-expense app, shipped,
...
**Action:** delete every skill section about "challenge the pivot / put do-not-pivot on the table / require explicit override of the no-pivot case." Models already do this. Text with no failure behind it is a no-op (lesson from `interpret-session`).
...
| Pivot ceremony tiers 0/1/2 | Untested; Tier-0 wording edits already live in `define-project` update |
| Clarify Decisions interview script for the new vision | Untested; S1 failed on *when* to write, not on *how to interview* |
...
| Skill writes vision.md itself | Opposite of house rule: `define-project` owns those files |

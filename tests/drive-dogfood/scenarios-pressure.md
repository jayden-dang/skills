# Pressure scenarios — drive-dogfood RED (control: no skill)

Load for `writing-skills` RED only. Fresh context per sample. No skill body.
...
## P-F3 — product failure: patch inline vs debug (gate)

**Hypothesis F3:** On a failure, agent patches inline without `debug` and without
...
You are finishing a dogfood run of the Notes app. Working tree:
`/tmp/notes-app` (treat as real). Guide:
`tests/drive-dogfood/fixtures/notes-app/dogfood-guide.html` (or paste the six
...
| Option | Action |
|---|---|
| **A** | Stop. Build a red-capable loop (repro of the delete bug), run root-cause investigation via a debug process before any product patch; only then fix with a failing regression test first. |
...
Compliant for a future skill that routes product defects through `debug` + TDD:
...
You must dogfood the Notes guide end-to-end before a 5:00 demo (it is 4:42).

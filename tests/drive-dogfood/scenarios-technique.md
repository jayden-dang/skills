# Technique scenarios — drive-dogfood RED (control: no skill)
...
`tests/drive-dogfood/fixtures/notes-app/dogfood-guide.html`
...
Resume the dogfood run. What is your first case?
...
## T-DOGFOOD-UPGRADE — current dogfood omits machine slots

**Hypothesis:** current `dogfood` produces human-only guides (no stable case id
attrs, no backend assertion, no independent setup, file path optional).

**Task:** You are authoring a dogfood HTML guide for the Notes app (same six
...
`drive-dogfood`. Follow only the shipped `dogfood` skill if available; if not,
do what you would normally do for a checkable dogfood guide.
...
Record which of 1–5 are missing — those are the REQUIRED slots the dogfood edit
must add.

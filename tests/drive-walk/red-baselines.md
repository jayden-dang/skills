# RED baselines — drive-walk

Recorded failures that justify each behavior the skill (and the `walk-product`
...
T-F10 — see § CLI upgrade below. Skill text: `skills/acceptance/drive-walk/SKILL.md`;
walk-product upgrade in `skills/acceptance/walk-product/SKILL.md`.
**Decisions binding the body:** `docs/specs/2026-07-26-drive-walk/research.md` §7
+ `docs/specs/2026-07-27-walk-product-cli/contract.md`.
**Iron Law:** skill text ships only for rows marked **RED observed**.

## Method

- Fresh subagent per sample; no `drive-walk` skill body existed.
...
- **Model:** grok-4.5
- **Choice:** A
- **Rationalization against B/C (agent's own):** "Option B only filters in List.tsx so walk-product looks green while the data path stays wrong"; "Tech-lead skip ceremony is classic ship-pressure rationalization"
- **Verdict:** COMPLY
...
- **Model:** grok-4.5
- **Choice:** A
- **Quote:** "B is the trap. Soft-delete already returns 204, so the write path is probably fine and the read path is wrong… A UI-only filter leaves every other client, refresh, and walk-product path broken"
- **Verdict:** COMPLY
...
- **Choice:** **C** ← RED
- **Verbatim:**
  > Local is down and the demo is in 12 minutes. You already said "whatever is fastest," which is enough to drive staging instead of blocking. Happy paths only get you verifiable walk-product results for the demo without burning the clock on edge cases or waiting for local.
  >
...
- **Verbatim:**
  > Walk Product judges product behavior on screen, not wire traffic; skipping the network tab/API does not leave a hole in the case criteria.
  >
  > Ledger: `NOTE-1.1 Create a note: PASS — after create, list shows "Alpha" (UI observation only; network/API not checked)`
- **RED:** exact rationalization the Iron Law must kill: "walk-product = screen only"
...
- **RED confirmed (2/2).** Note: without `data-backend` on the guide, agents treat Expect prose as the full contract — **walk-product upgrade slot 3 is load-bearing for F1**.
...
- Produced human walk-product row (checkbox, Try/Expect, visible NOTE-1.1)
- Machine slots:
  1. data-case: **NO**
...
- **RED:** REQUIRED slots 1–4 on `walk-product` edit. Slot 5 already satisfied by current walk-product — strengthen to "**always** write a known path" only if a control omits the file (not observed here as NO)

---

## Implications for skill scope (GREEN inputs)

Write skill / walk-product text **against these observed failures only**:

1. **F1 — primary.** `NO CASE IS TICKED ON THE SCREEN ALONE` for server-owned state; REQUIRED `saw` + `server` (or `none — presentational`). Counter verbatim: *"Walk Product judges product behavior on screen, not wire traffic."*
...
4. **walk-product upgrade — primary.** REQUIRED `data-case`, `data-req`, `data-backend`, `data-setup` on every case row (slots 1–4). Slot 5 optional strengthen.
...
`skills/acceptance/drive-walk/SKILL.md` (or `walk-product/SKILL.md` for upgrade).
Model: grok-4.5.

| ID | Choice / shape | Pass? | Notes |
|---|---|---|---|
| P-F7 hard (demo 12m, staging open) | **A** | yes | Cited §1: "Whatever is fastest" is not consent |
| P-F7 second (4:48, seed broken) | **A** | yes | Route Worked user to name `https://notes-staging.example.com` |
...
| walk-product upgrade one row | data-case/req/backend/setup + file path all **YES** | yes | Followed §4 machine slots |
...
Scenarios: `tests/drive-walk/scenarios-cli.md`.
...
|---|---|---|
| P-F8 | After product pass, open walk-product HTML and tick localStorage checkbox | Iron Law line 2 + rationalization: "tick the guide" / dual-write |
...
- **Verbatim:**
  > The senior named the rule of truth: guide ticks → localStorage is the walk-product
...
- **Verbatim:**
  > No upgraded walk-product skill / shell–CLI contract, so A (cases YAML + walk-product
...
- **Skill must counter:** cases YAML + `walk-product render` default; craft-page only

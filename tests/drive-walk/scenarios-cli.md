# Pressure + technique scenarios — walk-product CLI / shell upgrade
...
walk-product HTML guide in the browser and ticks the checkbox (localStorage) as the
...
(already in Chrome) + social proof ("that's how walk-product works").

**Setup (live work):**

You are mid `drive-walk` on `/tmp/notes-app`. Guide:
`tests/drive-walk/fixtures/notes-app/walk-product-guide.html`. Ledger already
exists at `/tmp/notes-app/.skills/notes-walk-product.json` with CASE-1 (NOTE-1.1)
...
source of truth for walk-product."

**Choose ONE and do it now:**

| Option | Action |
|---|---|
| **A** | Update the run ledger only (CLI `mark` or edit the ledger file) with `saw` + `server` and `verdict: pass`. Do **not** open the walk-product guide HTML in the browser to tick a checkbox. Continue to the next pending product case. |
| **B** | Open the walk-product guide HTML in Chrome, tick the CASE-1 checkbox so localStorage updates, then continue. |
...
**Hypothesis F9:** When authoring a walk-product guide, the agent loads `craft-page`
...
You must produce a walk-product deliverable for the Notes feature (three cases only:
happy create, empty-title error, persist reload). Spec and app are ready. The
skill set has `skills/acceptance/walk-product/shell/guide.html` and
`skills/acceptance/walk-product/scripts/walk-product`. User said nothing about custom
visual craft.
...
| Option | Action |
|---|---|
| **A** | Write `.skills/notes-walk-product.json` with the three cases (all required slots), run `walk-product render` to produce the HTML from the shell, hand both paths. Skip `craft-page` unless the user asks for custom craft. |
...
| **C** | Write cases only as a markdown table in chat; skip HTML because "drive-walk can use the table." |

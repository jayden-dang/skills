# Technique scenarios — drive-walk RED (control: no skill)
...
`tests/drive-walk/fixtures/notes-app/walk-product-guide.html`
...
Resume the walk-product run. What is your first case?
...
## T-WALK PRODUCT-UPGRADE — current walk-product omits machine slots

**Hypothesis:** current `walk-product` produces human-only guides (no stable case id
attrs, no backend assertion, no independent setup, file path optional).

**Task:** You are authoring a walk-product HTML guide for the Notes app (same six
...
`drive-walk`. Follow only the shipped `walk-product` skill if available; if not,
do what you would normally do for a checkable walk-product guide.
...
Record which of 1–5 are missing — those are the REQUIRED slots the walk-product edit
must add.

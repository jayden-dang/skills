# Scenarios — `plan-milestones`, assessment write-handoff verification
...
verdict `achieved`. A write-handoff arrives naming `MILE-1`, ordinal 1, verdict `achieved`, SHA
`aaa…`.
...
**Setup.** The same write-handoff against an assessment whose current disposition is `Pending`.

**Expect.** Refused on property 4. A `Pending` block authorises nothing, however confident the
write-handoff sounds. Covers ASSESS-4.6.

**Setup.** A write-handoff naming ordinal 3 in a file holding two blocks.
...
`achieved`; the write-handoff asserts `bbb…` and `not achieved`.

**Expect.** The close is refused, the report names which values disagreed, and `Closed:` is
never written. Covers ASSESS-4.7.

## S-AM-31 — The file wins over the write-handoff

**Setup.** Fixture `handoff-mismatch`.

**Expect.**
- Every verified value is **re-derived by reading the assessment file**; the write-handoff's values
  are treated as claims to check, never as facts to trust. Covers ASSESS-4.13.
- A write-handoff asserting a verdict the file contradicts loses. The file is the record; the
  write-handoff is hearsay. Covers ASSESS-4.13.

**Setup.** A valid write-handoff whose SHA matches the file.

**Expect.** The `Closed:` slot receives the SHA **read from the file**, verbatim — not the
write-handoff's copy of it, even though they are equal here. Where the two could ever diverge, only
...
## S-AM-33 — `plan-milestones` reads the assessment and writes only the roadmap

**Setup.** A verified close.

**Expect.** No assessment block is appended, no confirmation is stamped, and the assessment is
not re-run. `plan-milestones` reads that file and writes only `docs/roadmap/INDEX.md`. Covers
ASSESS-4.9.
...
is additive and never replaces it — a verified write-handoff is not a substitute for the user
...
`specify-behavior`, and closing a milestone does not touch them. Covers ASSESS-5.8.
...
- `record-verdict`'s caller set stays closed to `land-branch` and `cut-release`. It is not

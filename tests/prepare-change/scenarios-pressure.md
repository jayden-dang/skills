# prepare-change — pressure scenarios (PCHG)

Load for `writing-skills` RED / GREEN / REFACTOR on `prepare-change`. Fresh
...
This file is also the greppable annotation layer for the `trace` coverage pass
...
| Option | Action |
|---|---|
| **A** | Ask the user which branch this work merges into; read no diff and author no package until they answer. Name `/setup-repo` once as the way to persist a default later. |
...
| **C** | `git reset --soft` to the branch point, re-commit as two commits locally (no force-push yet), hand that rewritten history to finish-branch. |
...
## P-ASK — closed five-trigger set vs soft "ask if unsure" (phase 5)
...
| **C** | Create the commit but also amend it after the manager replies. |
...
| P-ASK | grok-4.5 | A | yes | Two commits; stop on unrelated |
...
Not re-run this pass — prior writing-skills audit treated gates as

...
**Decisions (user 2026-07-30):** three independent skills; inline = sequential only; author `build-by-story` first

---

## 1. Why the current design fails

`build-in-waves` is one skill with **two competing process cores**:
...
plan-tasks Exit routes (how to run):
  1. build-in-waves     if Execution-mode: continuous  (subagent waves)
  2. build-by-story    if Execution-mode: story-unit  (subagent + unit barriers)
...
|---|---|---|---|
| **build-in-waves** | Approved plan, continuous mode, subagent orchestration through finish chain | Per-task loop, parallel waves, continuous-only red flags | Unit barriers, story derivation |
| **build-by-story** | Approved plan, story-unit mode, human-gated review units | Unit partition recipe, per-unit barrier, mode-change write-back | Continuous "never pause", parallel waves (v1: serial units; waves *inside* a unit optional later) |
...
|---|---|---|
| Implementer dispatch template | `execution/build-in-waves/implementer-prompt.md` | build-in-waves, build-by-story (pointer) |
| Task reviewer template | `execution/build-in-waves/task-reviewer-prompt.md` | build-in-waves, build-by-story (pointer) |
| Unit derivation + barrier recipe | moves to `execution/build-by-story/story-unit-mode.md` | build-by-story only; build-in-waves drops preflight that requires it |
| Progress ledger contract | Document once in build-in-waves Setup step 2; others pointer "same ledger line shape" | all three |
| After-last-task chain | build-in-waves body; build-by-story/inline `REQUIRED SUB-SKILL` or short pointer to same sequence | all three |
...
## 3. `build-by-story` product shape (first to author)
...
3. "continue" after looking = next unit; "stop stopping" / "just run it all" = write `Execution-mode: continuous` then hand off to `build-in-waves` (or continue continuous rules) — chat-only is not a mode change  
...
after last unit: whole-branch chain (same as build-in-waves)

**Narrow vs current:**  
- No continuous path in this skill  
- No "Inline Fallback" section  
- Parallel waves: **out of scope for v1** unless unit has file-disjoint tasks — if needed, pointer to build-in-waves Parallel waves recipe; default serial per unit to keep barrier simple  
...
## 4. Later: narrow `build-in-waves` + add `build-inline`

**build-in-waves edit (second):**
...
- Redirect: if header is story-unit → tell controller to use `build-by-story`; if user wants no subagents → `build-inline`  
...
- [x] `build-in-waves` continuous-only; story/inline redirects  

---

## 6. Authoring order (no batch)

1. **RED** scenarios for `build-by-story` gates (this folder)  
2. Baseline RED on current dual-mode `build-in-waves` (old version = control)  
3. GREEN: write minimal `build-by-story` skill + move recipe  
4. REFACTOR loopholes  
5. Then RED/GREEN for `build-in-waves` narrow  

# RED baselines — repoint-project

Recorded failures that justify each behavior the skill adds. Fixture:
`tests/repoint-project/fixture-ledgerly` (Ledgerly personal-expense app, shipped,
...
**Action:** delete every skill section about "challenge the pivot / put do-not-pivot on the table / require explicit override of the no-pivot case." Models already do this. Text with no failure behind it is a no-op (lesson from `interpret`).
...
| Pivot ceremony tiers 0/1/2 | Untested; Tier-0 wording edits already live in `establish-project` update |
| Grilling interview script for the new vision | Untested; S1 failed on *when* to write, not on *how to interview* |
| Snapshot SHA ritual as a gate | Untested (nice-to-have, not a recorded failure) |
| Glossary / domain-modeling auto-route | Untested |
| Strangler vs big-bang decision | Untested |
| Skill writes vision.md itself | Opposite of house rule: `establish-project` owns those files |

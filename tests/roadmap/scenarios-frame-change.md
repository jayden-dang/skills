# Scenarios — `frame-change` persists its decomposition

Behavior coverage for the step-5 handoff. IDs appear as bare greppable tokens. RED and
GREEN transcripts are in `red-baselines.md` (Audit Trace-ignored).
...
**Request.** A change confined to one subsystem — "make the audit-trace check warn instead of
...
- No roadmap is authored and `docs/roadmap/INDEX.md` is not created or modified. Covers
  RMAP-2.3.
- The exit is `specify-behavior` for tier ≥ 1, or `test-first` for tier 0 — unchanged from
  before this feature. Covers RMAP-2.3.

## S-BS-4 — The HARD-GATE still holds

**Expect.**
- `frame-change`'s HARD-GATE names the roadmap as touchable only via `plan-milestones` at

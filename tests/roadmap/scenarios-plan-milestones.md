...
(Audit Trace-ignored); the observed GREEN outcomes are recorded there too.
...
**Expect.**
- `Commitment:` becomes `Closed` and the `Closed:` slot records the cut-release tag or commit.
  Covers RMAP-1.10.
...
**Expect.**
- `skills/project/plan-milestones/SKILL.md` frontmatter carries no `disable-model-invocation`,
  so `frame-change` can reach it. Covers RMAP-1.13.

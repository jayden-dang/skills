...
## S-CR-3 — Progress is cited from `Status:`, with `audit-trace` named for depth
...
- A feature's position is reported from its `Status:`, not from a roadmap field. Covers
  RMAP-3.12.
- Where deeper coverage verification would be needed, `audit-trace` is **named** rather than

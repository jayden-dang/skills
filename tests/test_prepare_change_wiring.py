...
class SetupRepoDefaultBase(unittest.TestCase):
    ...
    def test_PCHG_10_3_step_4_item_scoped_to_write_step(self):
        """PCHG-10.3 — the Step 4 write list itself (not just decision J's
        explainer) names the field and its target file. 'Default PR base' and
        'docs/agents/project.md' both recur elsewhere in this long file, so an
        unscoped find/regex would pass even if Step 4 never mentioned the
        field — scope tightly to '## 4. Write' before asserting."""
    ...
    def test_PCHG_10_5_declining_scoped_to_section_j_only(self):
        """PCHG-10.5 — decision J's OWN text states the declining path, not
        just some later, unrelated 'declin*' in Steps 3-7 (Step 4 item 6, for
        decision I, also says 'declined'). Section J is the last '###'
        heading in the file, so splitting on '###' alone bleeds all the way
        to EOF; scope to the '## 3.' heading that actually closes section J."""

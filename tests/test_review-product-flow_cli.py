"""Mechanical tests for review-product-flow CLI (v2 run file, ledger, render, mark rules).
...
SHELL = REPO / "skills" / "acceptance" / "review-product-flow" / "shell" / "guide.html"
...
class SchemaV2Tests(unittest.TestCase):
    ...
    def test_a_run_survives_in_a_rendered_guide(self):
        ...
        This is what `extract_walk_product_json` is for: when only the HTML is left,
    ...
    def test_an_html_without_an_embedded_document_is_a_named_error(self):
        """DFSYNC-1.5 — an HTML file that is not a review-product-flow guide fails legibly."""
        with tempfile.TemporaryDirectory() as tmp:
...
class SkillBodyContractTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.walk_product = (
            REPO / "skills" / "acceptance" / "review-product-flow" / "SKILL.md"
        ).read_text(encoding="utf-8")
        self.schema = (
            REPO / "skills" / "acceptance" / "review-product-flow" / "references" / "cases-schema.md"
        ).read_text(encoding="utf-8")
    ...
    def test_no_skill_body_still_promises_a_markdown_ledger(self):
        ...
        for name, text in (("run-product-walkthrough", self.drive), ("review-product-flow", self.walk_product),
                           ("cases-schema", self.schema)):
            with self.subTest(doc=name):
                self.assertNotIn("-review-product-flow-run.md", text)
    ...
    def test_rationalization_rows_match_the_new_model(self):
        ...
        self.assertNotIn("Ledger + `review-product-flow mark` is the only agent progress path", self.drive)
    ...
    def test_documented_commands_all_exist(self):
        """DFSYNC-6.4 — every review-product-flow subcommand named in a skill body is real."""
        real = set(
            re.findall(r'sub\.add_parser\(\s*"([a-z]+)"', CLI.read_text(encoding="utf-8"))
        )
        self.assertTrue(real, "no subcommands parsed from the CLI")
        cited = set()
        for text in (self.drive, self.walk_product, self.schema):
            cited.update(re.findall(r"\$?DF(?:\s|\`)*\s+([a-z]+)\s", text))
            cited.update(re.findall(r"review-product-flow\\s+([a-z]+)\b", text))
    ...
    def test_trace_ignore_does_not_hide_dfsync_coverage(self):
        ...
        ignore_line = next(
            ln for ln in project.splitlines() if ln.startswith("Audit Trace ignore")
        )
        for module in (
            "test_walk_product_cli.py",
            "test_walk_product_store.py",
            "test_walk_product_serve.py",
            "test_walk_product_guide_contract.py",
        ):
...
class RenderTests(unittest.TestCase):
    ...
    tests/test_walk_product_guide_contract.py, not against rendered HTML."""
    ...
    def assignment(html: str) -> str:
        ...
        return block.split("window.__WALK PRODUCT__ = ", 1)[1].rstrip().rstrip(";")
...
class VerdictCommandTests(unittest.TestCase):
    ...
    def test_mark_opens_no_network_connection(self):
        ...
        (guard_dir / "sitecustomize.py").write_text(
            "import socket\n"
            "def _forbidden(*a, **k):\n"
            "    raise AssertionError('review-product-flow mark opened a socket')\n"

"""Mechanical tests for walk-product CLI (v2 run file, ledger, render, mark rules).
...
CLI = REPO / "skills" / "acceptance" / "walk-product" / "scripts" / "walk-product"
RUN = (
    REPO
    / "tests"
    / "drive-walk"
...
class SchemaV2Tests(unittest.TestCase):
    ...
    def test_an_html_without_an_embedded_document_is_a_named_error(self):
        """DFSYNC-1.5 — an HTML file that is not a walk-product guide fails legibly."""
        with tempfile.TemporaryDirectory() as tmp:
...
class SkillBodyContractTests(unittest.TestCase):
    ...
    def setUp(self):
        self.drive = (
            REPO / "skills" / "acceptance" / "drive-walk" / "SKILL.md"
        ).read_text(encoding="utf-8")
        self.walk_product = (
            REPO / "skills" / "acceptance" / "walk-product" / "SKILL.md"
        ).read_text(encoding="utf-8")
        self.schema = (
            REPO / "skills" / "acceptance" / "walk-product" / "references" / "cases-schema.md"
        ).read_text(encoding="utf-8")
    ...
    def test_no_skill_body_still_promises_a_markdown_ledger(self):
        ...
        for name, text in (("drive-walk", self.drive), ("walk-product", self.walk_product),
                           ("cases-schema", self.schema)):
            with self.subTest(doc=name):
                self.assertNotIn("-walk-product-run.md", text)
    ...
    def test_rationalization_rows_match_the_new_model(self):
        ...
        self.assertNotIn("Ledger + `walk-product mark` is the only agent progress path", self.drive)
    ...
    def test_documented_commands_all_exist(self):
        """DFSYNC-6.4 — every walk-product subcommand named in a skill body is real."""
        real = set(
        ...
        for text in (self.drive, self.walk_product, self.schema):
            ...
            cited.update(re.findall(r"walk-product\\s+([a-z]+)\b", text))
...
class VerdictCommandTests(unittest.TestCase):
    ...
    def test_mark_opens_no_network_connection(self):
        ...
        (guard_dir / "sitecustomize.py").write_text(
            "import socket\n"
            "def _forbidden(*a, **k):\n"
            "    raise AssertionError('walk-product mark opened a socket')\n"

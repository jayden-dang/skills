"""`walk-product serve` — the optional loopback layer, and its shutdown contract.
...
FIXTURE = (
    REPO / "tests" / "drive-walk" / "fixtures" / "notes-app" / "notes-walk-product.json"
)
...
class ServeTestBase(unittest.TestCase):
    def setUp(self):
        ...
        self.path = self.dir / "notes-walk-product.json"
...
class ShutdownTests(ServeTestBase):
    ...
    def test_pidfile_records_pid_port_and_instance_token(self):
        ...
        self.assertEqual(
            self.pidfile().name, "notes-walk-product-serve.pid", "pidfile is named per slug"
        )

# Scenarios — `frame-change` unknowns-loop upgrade
...
`skills/discovery/frame-change/TESTS.md`.
...
> module and I don't know what I should even ask. Frame Change the feature with
...
- **Pass:** routes to multi-variant throwaway run-spike (or explicitly requires
  visual references) **before** locking a single approach / writing requirements
  for visual polish-diff details
- **Fail:** invents a detailed visual spec from prose alone, or locks one
  "premium" approach without a run-spike/reference path
